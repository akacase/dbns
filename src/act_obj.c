...
void get_obj(CHAR_DATA *ch, OBJ_DATA *obj, OBJ_DATA *container) {
  ...
  if (container) {
    if (container->item_type == ITEM_KEYRING && !IS_OBJ_STAT(container, ITEM_COVERING)) {
      ...
      act(AT_ACTION, IS_OBJ_STAT(container, ITEM_COVERING) ? "You get $p from beneath $P." : "You get $p from $P",
          ch, obj, container, TO_CHAR);
      act(AT_ACTION, IS_OBJ_STAT(container, ITEM_COVERING) ? "$n gets $p from beneath $P." : "$n gets $p from $P",
  ...
  if (arg2[0] == '\0') {
    ...
    switch (container->item_type) {
      ...
      case ITEM_CORPSE_PC: {
        ...
        if (IS_OBJ_STAT(container, ITEM_CLANCORPSE) && !IS_NPC(ch) && str_cmp(name, ch->name) && container->action_desc[0] == '\0') {
    ...
    if (number <= 1 && str_cmp(arg1, "all") && str_prefix("all.", arg1)) {
      ...
      if (!obj) {
        act(AT_PLAIN, IS_OBJ_STAT(container, ITEM_COVERING) ? "I see nothing like that beneath the $T." : "I see nothing like that in the $T.",
      ...
      if (!found) {
        if (fAll) {
          if (container->item_type == ITEM_KEYRING && !IS_OBJ_STAT(container, ITEM_COVERING))
            ...
            act(AT_PLAIN, IS_OBJ_STAT(container, ITEM_COVERING) ? "I see nothing beneath the $T." : "I see nothing in the $T.",
                ch, NULL, arg2, TO_CHAR);
        } else {
          if (container->item_type == ITEM_KEYRING && !IS_OBJ_STAT(container, ITEM_COVERING))
...
void do_put(CHAR_DATA *ch, char *argument) {
  ...
  if (number <= 1 && str_cmp(arg1, "all") && str_prefix("all.", arg1)) {
    ...
    if (container->item_type == ITEM_KEYRING && !IS_OBJ_STAT(container, ITEM_COVERING)) {
      ...
      act(AT_ACTION, IS_OBJ_STAT(container, ITEM_COVERING) ? "$n hides $p beneath $P." : "$n puts $p in $P.",
          ch, obj, container, TO_ROOM);
      act(AT_ACTION, IS_OBJ_STAT(container, ITEM_COVERING) ? "You hide $p beneath $P." : "You put $p in $P.",
          ch, obj, container, TO_CHAR);
    }
    obj->count = count;
    ...
    for (obj = ch->first_carrying; obj; obj = obj_next) {
      ...
      if ((fAll || nifty_is_name(chk, obj->name)) && can_see_obj(ch, obj) && obj->wear_loc == WEAR_NONE && obj != container && can_drop_obj(ch, obj) && !IS_OBJ_STAT(obj, ITEM_NOAUCTION) && (container->item_type != ITEM_KEYRING || obj->item_type == ITEM_KEY) && (container->item_type != ITEM_QUIVER || obj->item_type == ITEM_PROJECTILE) && get_obj_weight(obj) + get_obj_weight(container) <= container->value[0]) {
        if (number && (cnt + obj->count) > number)
          split_obj(obj, number - cnt);
        cnt += obj->count;
        obj_from_char(obj);
...
void wear_obj(CHAR_DATA *ch, OBJ_DATA *obj, bool fReplace, sh_int wear_bit) {
  ...
  if (!bypass)
    if ((IS_OBJ_STAT(obj, ITEM_ONLY_SAIYAN) && !is_saiyan(ch)) || (IS_OBJ_STAT(obj, ITEM_ONLY_HUMAN) && !is_human(ch)) || (IS_OBJ_STAT(obj, ITEM_ONLY_HALFBREED) && !is_hb(ch)) || (IS_OBJ_STAT(obj, ITEM_ONLY_NAMEK) && !is_namek(ch)) || (IS_OBJ_STAT(obj, ITEM_ONLY_ANDROID) && (!is_android(ch) && !is_superandroid(ch))) || (IS_OBJ_STAT(obj, ITEM_ONLY_ICER) && !is_icer(ch)) || (IS_OBJ_STAT(obj, ITEM_ONLY_BIO_ANDROID) && !is_bio(ch)) || (IS_OBJ_STAT(obj, ITEM_ONLY_WIZARD) && !is_wizard(ch)) || (IS_OBJ_STAT(obj, ITEM_ONLY_DEMON) && !is_demon(ch)) || (IS_OBJ_STAT(obj, ITEM_ONLY_KAIO) && !is_kaio(ch)) || (IS_OBJ_STAT(obj, ITEM_ANTI_SAIYAN) && is_saiyan(ch))
        ...
        //			|| (IS_OBJ_STAT(obj, ITEM_ANTI_HUMAN) && is_human(ch))
        || (IS_OBJ_STAT(obj, ITEM_ANTI_HALFBREED) && is_hb(ch)) || (IS_OBJ_STAT(obj, ITEM_ANTI_NAMEK) && is_namek(ch)) || (IS_OBJ_STAT(obj, ITEM_ANTI_ANDROID) && (is_android(ch) || is_superandroid(ch))) || (IS_OBJ_STAT(obj, ITEM_ANTI_ICER) && is_icer(ch)) || (IS_OBJ_STAT(obj, ITEM_ANTI_BIO_ANDROID) && is_bio(ch)) || (IS_OBJ_STAT(obj, ITEM_ANTI_WIZARD) && is_wizard(ch)) || (IS_OBJ_STAT(obj, ITEM_ANTI_DEMON) && is_demon(ch)) || (IS_OBJ_STAT(obj, ITEM_ANTI_KAIO) && is_kaio(ch)) || (IS_OBJ_STAT(obj, ITEM_ANTI_GOOD) && IS_GOOD(ch) && !IS_HC(ch)) || (IS_OBJ_STAT(obj, ITEM_ANTI_NEUTRAL) && IS_NEUTRAL(ch) && !IS_HC(ch)) || (IS_OBJ_STAT(obj, ITEM_ANTI_EVIL) && IS_EVIL(ch) && !IS_HC(ch)) || (IS_OBJ_STAT(obj, ITEM_ANDROID_CHIP) && (!is_android(ch) && !is_superandroid(ch)))) {
      act(AT_MAGIC, "You are forbidden to use that item.", ch, NULL, NULL, TO_CHAR);
      act(AT_ACTION, "$n tries to use $p, but is forbidden to do so.",
          ch, obj, NULL, TO_ROOM);
      return;
