...
void do_mstat(CHAR_DATA *ch, char *argument) {
  ...
  if (!IS_NPC(victim) && victim->pcdata->clan)
    pager_printf_color(ch, "   &c%s: &w%s",
                       victim->pcdata->clan->clan_type ==
                               CLAN_ORDER
                           ? "Order"
                       : victim->pcdata->clan->clan_type == CLAN_GUILD ? "Guild"
                                                                       : "Clan",
  ...
  pager_printf_color(ch,
                     ...
                     victim->speaks, victim->speaking, (get_trust(ch) < 59 && !IS_NPC(victim)) ? "0" : num_punct_ld(victim->exp));
  if (IS_NPC(victim))
    ...
    for (d = first_descriptor; d; d = d->next)
      if ((d->connected == CON_PLAYING || d->connected == CON_EDITING) && (victim = d->character) != NULL && !IS_NPC(victim) && victim->in_room && can_see(ch, victim) && victim->fighting && victim->level >= low && victim->level <= high) {
        ...
        pager_printf_color(ch,
                           ...
                           IS_NPC(victim->fighting->who) ? victim->fighting->who->pIndexData->vnum : 0, victim->in_room->area->name,
                           victim->in_room ==
                                   NULL
                               ? 0
                               : victim->in_room->vnum);
    ...
    for (victim = first_char; victim; victim = victim->next)
      if (IS_NPC(victim) && victim->in_room && can_see(ch, victim) && victim->hating && victim->level >= low && victim->level <= high) {
        ...
        pager_printf_color(ch,
                           ...
                           IS_NPC(victim->hating->who) ? victim->hating->who->pIndexData->vnum : 0,
                           victim->in_room->area->name,
                           victim->in_room ==
                                   NULL
                               ? 0
  ...
  for (obj = first_object; obj; obj = obj->next) {
    if (obj->in_room && !str_cmp(buf2, obj->short_descr) && (obj->pIndexData->vnum == 11)) {
      ...
      pager_printf_color(ch,
                         ...
                         capitalize(arg1), IS_OBJ_STAT(obj, ITEM_CLANCORPSE) ? "&RPK" : "&R  ", obj->in_room->area->name,
                         obj->in_room->vnum,
...
void do_oinvoke(CHAR_DATA *ch, char *argument) {
  ...
  if (!IS_OBJ_STAT(pObjIndex, ITEM_PROTOTYPE) && get_trust(ch) < sysdata.level_modify_proto) {
...
void do_users(CHAR_DATA *ch, char *argument) {
  ...
  for (d = first_descriptor; d; d = d->next) {
    if (arg[0] == '\0') {
      ...
      if ((get_trust(ch) >= LEVEL_SUPREME || (d->character && can_see(ch, d->character))) && (!str_prefix(arg, d->host) || (d->character && !str_prefix(arg, d->character->name)))) {
        count++;
        if (d->compressing)
          mccpCount++;
        pager_printf(ch,
...
void do_force(CHAR_DATA *ch, char *argument) {
  ...
  if (!str_cmp(arg, "all")) {
    ...
    if (mobsonly && IS_NPC(victim) && !xIS_SET(victim->act, ACT_PROTOTYPE) && get_trust(ch) < sysdata.level_modify_proto) {
      send_to_char("You don't have the power to do that.\n\r", ch);
      return;
    }
...
void do_destroy(CHAR_DATA *ch, char *argument) {
  ...
  if (!rename(buf, buf2)) {
    ...
    if (!remove(buf))
      ...
      ch_printf(ch,
                "Unknown error #%d - %s (housing data)."
                "  Report to Coder.\n\r",
                errno, strerror(errno));
      sprintf(buf2, "%s destroying %s", ch->name, buf);
      perror(buf2);
...
void do_setweather(CHAR_DATA *ch, char *argument) {
  ...
  if (arg[0] == '\0') {
    ...
    ch_printf(ch, "\n\rResulting values:\n\r");
    ch_printf(ch,
              "Weather variables range from "
              "%d to %d.\n\r",
              -3 * weath_unit, 3 * weath_unit);
    ch_printf(ch,
              "Weather vectors range from "
              "%d to %d.\n\r",
              -1 * max_vector, max_vector);
    ch_printf(ch,
              "The maximum a vector can "
...
void do_project(CHAR_DATA *ch, char *argument) {
  ...
  if (!str_cmp(arg1, "status")) {
    if (pproject->owner && str_cmp(pproject->owner, ch->name) &&
        get_trust(ch) < LEVEL_GREATER && pproject->coder && str_cmp(pproject->coder, ch->name) &&
        str_cmp(ch->pcdata->council_name, "Code Council")) {
