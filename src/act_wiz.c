...
void
do_showimmlog(CHAR_DATA * ch, char *argument)
{
	...
	if (arg[0] == '\0'
	    || lstart[0] == '\0'
	    || lcount[0] == '\0' || !is_number(lstart) || !is_number(lcount)) {
		send_to_pager
		    ("   showimmlog <target> <starting line> <number of lines>\n\r", ch);
		return;
	} else {
		start = atoi(lstart);
...
do_watch(CHAR_DATA * ch, char *argument)
{
	...
	if (arg[0] == '\0' || !str_cmp(arg, "help")) {
		...
		/*
		 * Only IMP+ can see all the watches. The rest can just see
		 * their own.
		 */
	...
	/*
	 * Display all watches, only IMP+ can see all the watches. The rest
	 * can just see their own.
	 */
	if (get_trust(ch) >= LEVEL_IMPLEMENTOR
			...
			for (pw = first_watch; pw; pw = pw->next)
				if (!str_cmp(ch->name, pw->imm_name))
					if (num == ++cou) {
						...
						/*
						 * Now we can unlink and
						 * then clear up that final
						 * pointer
						 */
	...
	if (!str_cmp(arg, "player") && *arg2) {
		...
		/*
		 * stupid get_char_world returns ptr to "samantha" when
		 * given "sam"
		 */
		/*
		 * so I do a str_cmp to make sure it finds the right player
		 * --Gorog
		 */
		sprintf(buf, "0.%s", arg2);
		/* if vic is in game now */
		if ((vic = get_char_world(ch, buf)))
			if ((!IS_NPC(vic)) && !str_cmp(arg2, vic->name))
				SET_BIT(vic->pcdata->flags, PCFLAG_WATCH);
		/* ins new watch if app */
		if (first_watch)
			for (pw = first_watch; pw; pw = pw->next)
...
void
no_npc(CHAR_DATA * ch) {
     send_to_char("can't be used on NPC.", ch);
}
...
do_bamfin(CHAR_DATA * ch, char *argument)
{
	...
	if (!argument || argument[0] == '\0')
		send_to_char("usage: bamfin <string>.\n\r", ch);
	if (!IS_NPC(ch)) {
...
do_bamfout(CHAR_DATA * ch, char *argument)
{
	if (IS_NPC(ch))
	     no_npc(ch);
	if (!argument || argument[0] == '\0') {
		send_to_char("usage: bamfout <string>.\n\r", ch);
	}
	if (!IS_NPC(ch)) {
		smash_tilde(argument);
...
echo_to_all(sh_int AT_COLOR, char *argument, sh_int tar)
{
	...
	for (d = first_descriptor; d; d = d->next) {
		/*
		 * Added showing echoes to players who are editing, so they
		 * won't miss out on important info like upcoming reboots.
		 * --Narn
		 */
...
void
echo_to_area(AREA_DATA * area, char *argument)
{
	CHAR_DATA *vic;
...
do_transfer(CHAR_DATA * ch, char *argument)
{
	...
	if (get_trust(ch) < get_trust(victim) && !IS_NPC(victim)) {
		send_to_char
		    ("You better not, that person is more goddly than you.\n\r", ch);
		return;
	}
...
do_ostat(CHAR_DATA * ch, char *argument)
{
	...
	ch_printf_color(ch, "&cWeight: &w%s/%s   ", num_punct(obj->weight),
	    num_punct(get_obj_weight(obj)));
	...
	if (obj->item_type == ITEM_ARMOR && obj->value[3] > 0) {
		...
		plMod = plMod / 10000;
		ch_printf_color(ch,
		    "&W *Object Value 3 is giving a %f%% bonus to PL gains.\n\r",
		    plMod * 100);
	}
...
do_mstat(CHAR_DATA * ch, char *argument)
{
	...
	pager_printf_color(ch,
	    "&cHps     : &w%-12s    &cMana   : &w%-12s    &cArmor     : &w%-12s\n\r",
	    hpbuf, mnbuf, mvbuf);
	pager_printf_color(ch,
	    "&cHitroll : &CN/A             &cAlign  : &w%-5d           &cDamage %% : &w%d\n\r",
	...
	if (IS_NPC(victim))
		pager_printf_color(ch,
		    "&cMob hitdie : &C%dd%d+%d    &cMob damdie : &C%dd%d+%d          &cNumAttacks(extra) : &C%d\n\r",
	...
	if (!IS_NPC(victim))
		...
		send_to_pager("\n\r", ch);
	pager_printf_color(ch,
	    "&cSave versus: &w%d %d %d %d %d       &cItems: &w(%d/%d)  &cWeight &w(%s/%s)\n\r",
	    ...
	    can_carry_n(victim), num_punct(victim->carry_weight),
	    num_punct(can_carry_w(victim)));
	pager_printf_color(ch,
	    "&cYear: &w%-5d  &cSecs: &w%d  &cTimer: &w%d  &cZeni: &Y%s\n\r",
	...
	pager_printf_color(ch,
	    "&cSpeaks: &w%d   &cSpeaking: &w%d   &cPowerLevel: &w%s",
	    victim->speaks, victim->speaking, (get_trust(ch) < 59
		&& !IS_NPC(victim))
	    ? "0" : num_punct_ld(victim->exp));
	if (IS_NPC(victim))
		...
		for (d = first_descriptor; d; d = d->next)
			if ((d->connected == CON_PLAYING || d->connected == CON_EDITING)
				...
				pager_printf_color(ch,
				    ...
				    IS_NPC(victim->fighting->
					who) ? victim->fighting->who->pIndexData->
...
do_oinvoke(CHAR_DATA * ch, char *argument)
{
	...
	if ((pObjIndex->item_type == ITEM_DRAGONBALL ||
		pObjIndex->item_type == ITEM_DRAGONRADAR) && ch->level < 63) {
...
do_purge(CHAR_DATA * ch, char *argument)
{
	...
	if (arg[0] == '\0') {
		...
		save_house_by_vnum(ch->in_room->vnum);	/* Prevent House Object
							 * Duplication */
		return;
	}
	victim = NULL;
	...
	/*
	 * fixed to get things in room first -- i.e., purge portal (obj), no
	 * more purging mobs with that keyword in another room first -- Tri
	 */
	...
	if (obj) {
		...
		act(AT_IMMORT, "You make $p disappear in a puff of smoke!", ch, obj,
		    NULL, TO_CHAR);
		extract_obj(obj);
		save_house_by_vnum(vnum);	/* Prevent House Object
						 * Duplication */
		return;
	}
	if (!IS_NPC(victim)) {
...
do_balzhur(CHAR_DATA * ch, char *argument)
{
	...
	for (pArea = first_build; pArea; pArea = pArea->next)
		if (!str_cmp(pArea->filename, buf2)) {
			...
			set_char_color(AT_RED, ch);	/* Log message changes
							 * colors */
			if (!rename(buf, buf2))
				send_to_char
...
do_advance(CHAR_DATA * ch, char *argument)
{
	...
	/*
	 * Lower level: Reset to level 1. Then raise again. Currently, an
	 * imp can lower another imp. -- Swiftest Can't lower imms >= your
	 * trust (other than self) per Narn's change. Few minor text changes
	 * as well.  -- Blod
	 */
	if (level <= victim->level) {
		...
		set_char_color(AT_IMMORT, victim);
		/*
		 * Fixed bug here, victim->level has not been changed here,
		 * thus their level is STILL going to be above LEVEL_AVATAR,
		 * changed to just 'level' as that's what they're being
		 * advanced to. - Orion Elder
		 */
		if (level <= LEVEL_AVATAR && IS_IMMORTAL(victim)) {
			...
			if (!IS_RETIRED(victim)) {
				/*
				 * Fixed bug here, was removing the immortal
				 * data of the person who used advance -
				 * Orion Elder
				 */
				sprintf(buf, "%s%s", GOD_DIR, capitalize(victim->name));

				/*
				 * Added to notify of removal of Immortal
				 * data. - Orion Elder
				 */
				if (!remove(buf))
					send_to_char("Player's immortal data destroyed.\n\r", ch);
		...
		/*
		 * Stuff added to make sure character's wizinvis level
		 * doesn't stay higher than actual level, take wizinvis away
		 * from advance < 50
		 */
...
do_strew(CHAR_DATA * ch, char *argument)
{
	...
	if (!str_cmp(arg2, "zeni")) {
		...
		victim->gold = 0;
		act(AT_MAGIC,
		    "$n gestures and an unearthly gale sends $N's zeni flying!", ch,
		    NULL, victim, TO_NOTVICT);
		act(AT_MAGIC,
		    "You gesture and an unearthly gale sends $N's zeni flying!", ch,
		    NULL, victim, TO_CHAR);
		act(AT_MAGIC,
		...
		for (vch = first_char; vch; vch = vch_next) {
			...
			if (!IS_NPC(vch)) {
				...
				if (str_cmp(ch->name, "saiyr"))
					...
					act(AT_LBLUE,
					    "$n tosses soapy water on you, and you feel refreshed!",
					    ch, NULL, vch, TO_VICT);
			}
		}
...
do_notitle(CHAR_DATA * ch, char *argument)
{
	...
	if (IS_SET(victim->pcdata->flags, PCFLAG_NOTITLE)) {
		...
		sprintf(buf, "the %s", title_table[victim->class][victim->level]
...
do_silence(CHAR_DATA * ch, char *argument)
{
	...
	if (xIS_SET(victim->act, PLR_SILENCE)) {
		...
		if (value > 480) {
			...
			sprintf(log_buf,
			    "Log: %s silenced %s for more than 8 hours (%d minutes)",
			    ch->name, victim->name, value);
			log_string_plus(log_buf, LOG_NORMAL, LEVEL_IMMORTAL);
		}
		sprintf(buf, "%s on %.24s", capitalize(ch->name),
		    ctime(&current_time));
		ch->pcdata->silencedby = STRALLOC(buf);
		xSET_BIT(victim->act, PLR_SILENCE);
		ch_printf(victim,
		    "You can't use channels!  You have been silenced for %d minutes\n\r",
		    value);
...
do_fixchar(CHAR_DATA * ch, char *argument)
{
	...
	fix_char(victim);
	/*
	 * victim->armor  = 100; victim->mod_str = 0; victim->mod_dex = 0;
	 * victim->mod_wis = 0; victim->mod_int = 0; victim->mod_con = 0;
	 * victim->mod_cha = 0; victim->mod_lck = 0; victim->damroll = 0;
	 * victim->hitroll = 0; victim->alignment       = URANGE(-1000,
	 * victim->alignment, 1000); victim->saving_spell_staff = 0;
	 */
	send_to_char("Done.\n\r", ch);
}
...
void
do_settime(CHAR_DATA * ch, char *argument)
{
	char 	arg[MAX_INPUT_LENGTH];
	char 	arg2[MAX_INPUT_LENGTH];
...
do_form_password(CHAR_DATA * ch, char *argument)
{
	...
	for (p = pwcheck; *p != '\0'; p++) {
		if (*p == '~') {
			send_to_char
			    ("New password not acceptable, cannot use the ~ character.\n\r", ch);
			return;
		}
	}
		...
		for (rid = room_index_hash[icnt]; rid; rid = rid_next) {
			...
			for (exit = rid->first_exit; exit; exit = exit_next) {
				...
				if (rid->area == pArea || exit->to_room->area == pArea) {
					...
					DISPOSE(exit);
					/*
					 * Crash bug fix.  I know it could
					 * go from the start several times
					 * But you CAN NOT iterate over a
					 * link-list and DELETE from it or
...
do_destroy(CHAR_DATA * ch, char *argument)
{
	...
	if (!rename(buf, buf2)) {
		...
		if (!remove(buf))
			...
			ch_printf(ch,
			    "Unknown error #%d - %s (immortal data).  Report to Goku.\n\r",
			    errno, strerror(errno));
			sprintf(buf2, "%s destroying %s", ch->name, buf);
			perror(buf2);
		}
	...
	if (strchr(argument, '#')) {
		...
		for (i = 0; i < MAX_KEY_HASH; i++)
			for (room = room_index_hash[i]; room; room = room->next) {
				...
				if (fEverywhere)	/* Everywhere executes
							 * always */
					found = true;
				else if (!room->first_person)	/* Skip it if room is
								 * empty */
					continue;
				/* ->people changed to first_person -- TRI */

				/*
				 * Check if there is anyone here of the
				 * requried type
				 */
				/*
				 * Stop as soon as a match is found or there
				 * are no more ppl in room
				 */
				/* ->people to ->first_person -- TRI */
				for (p = room->first_person; p && !found; p = p->next_in_room) {

					if (p == ch)	/* do not execute on
							 * oneself */
						continue;
				...
				if (found && !room_is_private(room)) {
					/*
					 * This may be ineffective. Consider
					 * moving character out of old_room
					 * once at beginning of command then
					 * moving back at the end. This
					 * however, is more safe?
					 */
...
do_cset(CHAR_DATA * ch, char *argument)
{
	...
	if (argument[0] == '\0') {
		...
		pager_printf_color(ch,
		    "&wDays before purging regular players: &W%d\n\r",
		...
		pager_printf_color(ch, "  &wAHelp:                        &W%-7s",
		    (sysdata.ahelp) ? "ON" : "off");
		pager_printf_color(ch, "  &wMax Player Check is:          &W%-7s\n\r",
		    (sysdata.check_plimit) ? "ON" : "off");
		pager_printf_color(ch, "  &wMax Online Players:           &W%-7d",
		    sysdata.plimit);
		pager_printf_color(ch, "&wNo Max Player Level:           &W%d\n\r",
		    sysdata.level_noplimit);
		pager_printf_color(ch, "  &wHiscore Tables:               &W%-7s",
		    (sysdata.stall_hiscores) ? "ON" : "off");
...
do_hell(CHAR_DATA * ch, char *argument)
{
	...
	/* used to save ch, fixed by Thoric 09/17/96 */
	save_char_obj(victim);
}
...
do_setclass(CHAR_DATA * ch, char *argument)
{
	...
	if (!str_cmp(arg2, "create")) {
		...
		MAX_PC_CLASS++;
		sprintf(classlist, "%s", CLASS_LIST);
...
do_showrace(CHAR_DATA * ch, char *argument)
{
	...
	sprintf(buf,
	    "Str Plus: %-3d\tDex Plus: %-3d\tWis Plus: 0\tInt Plus: %-3d\t\n\r",
...
do_ipcompare(CHAR_DATA * ch, char *argument)
{
	...
	if (!str_cmp(arg, "total")) {
		IPCOMPARE_DATA *first_ip = NULL, *last_ip = NULL, *hmm,
		       *hmm_next;
			...
			if (fMatch && hmm) {
				...
				if (!hmm->printed && hmm->count > 0) {
					sprintf(buf,
					    " %3d| %2d|%4d|%6d| %-12s",
					    hmm->descriptor,
					    hmm->connected, hmm->idle / 4, hmm->port, hmm->name);
					if (get_trust(ch) >= LEVEL_SAVIOR)
...
do_pcrename(CHAR_DATA * ch, char *argument)
{
	...
	/*
	 * Just a security precaution so you don't rename someone you don't
	 * mean too --Shaddai
	 */
	if ((victim = get_char_room(ch, arg1)) == NULL) {
...
void
do_pclastrename(CHAR_DATA * ch, char *argument)
{
	CHAR_DATA *victim;
	char 	arg1[MAX_INPUT_LENGTH];
	char 	arg2[MAX_INPUT_LENGTH];
 ...
 */
void
do_allhigh(CHAR_DATA * ch, char *argument)
{
	CHAR_DATA *vch;
 ...
 */
void
do_godset(CHAR_DATA * ch, char *argument)
{
	char 	arg1[MAX_INPUT_LENGTH];
	char 	arg2[MAX_INPUT_LENGTH];
	char 	arg3[MAX_INPUT_LENGTH];
	...
	if (!str_cmp(arg2, "basepl") || !str_cmp(arg2, "currentpl")) {
		...
		if (is_number(arg3))
			pager_printf_color(ch, "&wGODSETING %s's %s to %d.\n\r",
			    victim->name, arg2, value);
		else
			pager_printf_color(ch, "&wGODSETING %s's %s to %s.\n\r",
			    victim->name, arg2, arg3);
	}
	...
	if (!str_cmp(arg2, "currentpl")) {
		...
		if (valueld < 1) {
			ch_printf(ch,
			    "Current powerlevel setting must be greater than 0.\n\r");
			return;
		}
		if (valueld > pow(10, 30)) {
	...
	if (!str_cmp(arg2, "affected")) {
		...
		if (!argument || argument[0] == '\0') {
			send_to_char
			    ("usage: godset <victim> affected <flag> [flag]...\n\r", ch);
			return;
		}
		while (argument[0] != '\0') {
			argument = one_argument(argument, arg3);
 ...
 */
void
do_godstat(CHAR_DATA * ch, char *argument)
{
	char 	arg[MAX_STRING_LENGTH];
	CHAR_DATA *victim;
	char 	buf[MAX_STRING_LENGTH];
	...
	pager_printf_color(ch, "\n\r&G%s%s%s.&C\n\r", victim->name,
	    victim->pcdata->last_name ? victim->
	    pcdata->last_name : "", victim->pcdata->title);
	...
	pager_printf_color(ch,
	    "RACE : &W%-10.10s&C                           Played: &w%d&C hours\n\r",
	    capitalize(get_race(victim)),
	    (get_age(victim) - 4) * 2);

	pager_printf_color(ch,
	    "YEARS: &W%-6d&C        SEX: &W%-7s&C           Log In: %s\r",
	    get_newage(victim),
	    victim->sex == SEX_MALE ? "MALE" : victim->sex ==
	    SEX_FEMALE ? "FEMALE" : "NEUTRAL",
	    ctime(&(victim->logon)));
	...
	if (victim->level >= 15 || IS_PKILL(ch)) {
		pager_printf_color(ch,
		    "STR  : &W%2.2d&C(&w%2.2d&C)    HitRoll: &R%-4d&C              Saved:  %s\r",
		    get_curr_str(victim), victim->perm_str,
		    GET_HITROLL(victim),
		    victim->save_time ? ctime(&(victim->save_time)) :
		    "no save this session\n");

		pager_printf_color(ch,
		    "INT  : &W%2.2d&C(&w%2.2d&C)    DamRoll: &R%-4d&C              Time:   %s\r",
		    get_curr_int(victim), victim->perm_int,
		    GET_DAMROLL(victim), ctime(&current_time));
	} else {
		pager_printf_color(ch,
		    "STR  : &W%2.2d&C(&w%2.2d&C)                               Saved:  %s\r",
		    get_curr_str(victim), victim->perm_str,
		    victim->save_time ? ctime(&(victim->save_time)) :
		    "no\n");

		pager_printf_color(ch,
		    "INT  : %&W2.2d&C(&w%2.2d&C)                               Time:   %s\r",
		    get_curr_int(victim), victim->perm_int,
		    ctime(&current_time));
	}
	...
	if (ch->level < LEVEL_AVATAR)
		pager_printf_color(ch,
		    "DEX  : &W%2.2d&C(&w%2.2d&C)      Align: %-20.20s&C      Items: &W%3d&C  (&wmax %3d&C)\n\r",
		    get_curr_dex(victim), victim->perm_dex, buf,
		...
		pager_printf_color(ch,
		    ...
		    victim->alignment, buf, victim->carry_number,
		    can_carry_n(victim));
	...
	case POS_FIGHTING:
		...
		break;
	case POS_EVASIVE:
		sprintf(buf, "fighting (evasive)");	/* Fighting style
							 * support -haus */
		break;
	case POS_DEFENSIVE:
	...
	pager_printf_color(ch,
	    "CON  : &W%2.2d&C(&w%2.2d&C)      Pos'n: &W%-21.21s&C  Weight: &W%3d&C  (&wmax %3d&C)\n\r",
	    get_curr_con(victim), victim->perm_con, buf,
	...
	pager_printf_color(ch,
	    "ZENI : &Y%-13s&C                                            AutoSac: (&W%c&C)\n\r",
	    num_punct(victim->gold), xIS_SET(victim->act,
		PLR_AUTOSAC) ? 'X' :
	    ' ');
	...
	pager_printf_color(ch,
	    "                         PKills: [&R%5.5d&C]  IllegalPKs: [&R%3.3d&C]  MKills: [&R%5.5d&C]\n\r",
	    victim->pcdata->pkills, victim->pcdata->illegal_pk,
	    victim->pcdata->mkills);
	pager_printf_color(ch,
	    "                       SparWins: [&R%5.5d&C]     PDeaths: [&R%3.3d&C] MDeaths: [&R%5.5d&C]\n\r",
	    victim->pcdata->spar_wins, victim->pcdata->pdeaths,
	    victim->pcdata->mdeaths);
	pager_printf_color(ch,
	    "                       SparLoss: [&R%5.5d&C]      Self Destruct Charges: [&R%5.5d&C]\n\r",
	...
	if (victim->morph && victim->morph->morph) {
		...
		if (IS_IMMORTAL(victim))
			pager_printf(ch, "Morphed as (%d) %s with a timer of %d.\n\r",
			    victim->morph->morph->vnum,
			    victim->morph->morph->short_desc, victim->morph->timer);
		else
	...
	if (victim->pcdata->clan && victim->pcdata->clan->clan_type != CLAN_ORDER
		...
		pager_printf(ch,
		    ...
		    victim->pcdata->clan->name,
		    victim->pcdata->clan->pkills[6],
		    (victim->pcdata->clan->pkills[1] +
			victim->pcdata->clan->pkills[2] +
			victim->pcdata->clan->pkills[3] +
			victim->pcdata->clan->pkills[4] +
	...
	if (victim->pcdata->clan && victim->pcdata->clan->clan_type == CLAN_ORDER) {
		...
		pager_printf(ch,
		    ...
		    victim->pcdata->clan->name, victim->pcdata->clan->mkills,
	...
	if (IS_IMMORTAL(victim)) {
		...
		pager_printf(ch, "IMMORTAL DATA:  Wizinvis [%s]  Wizlevel (%d)\n\r",
		    xIS_SET(victim->act, PLR_WIZINVIS) ? "X" : " ",
		...
		if (victim->pcdata->area) {
			...
			pager_printf(ch, "Area Loaded [%s]\n\r",
			    (IS_SET(victim->pcdata->area->status, AREA_LOADED)) ?
			    "yes" : "no");
		}
	}
			...
			if (ch->level >= 20) {
				if (paf->modifier == 0)
					...
					pager_printf(ch, "[%-15.15s; %7.7s;%5d rds]    ",
					    sktmp->name,
					    tiny_affect_loc_name(paf->location), paf->duration);
				else
	...
	pager_printf(ch,
	    "Silence time left: %d   Safe from bounty time left:      %d\n\r",
...
void
do_godwho(CHAR_DATA * ch, char *argument)
{
	DESCRIPTOR_DATA *d;
	sh_int 	cswitch = 1;
	CHAR_DATA *vch;
	...
	for (d = first_descriptor; d; d = d->next) {
		...
		if (vch->sex == sex || sex == -1) {
			if (cswitch == 1) {
				pager_printf_color(ch,
				    "&B|&W[%10s][HOURS][%3d] Base/Current PL:[%15s]/",
				    vch->name, ((get_age(vch) - 4) * 2),
				    num_punct_ld(vch->exp));
				pager_printf_color(ch, "&W[%15s]Energy:[%6s]&B|\n\r",
				    num_punct_ld(vch->pl),
				    num_punct(vch->max_mana));
				cswitch = 2;
			} else {
				pager_printf_color(ch,
				    "&B|&Y[%10s][HOURS][%3d] Base/Current PL:[%15s]/",
				    vch->name, ((get_age(vch) - 4) * 2),
				    num_punct_ld(vch->exp));
 ...
 */
void
do_reimburse(CHAR_DATA * ch, char *argument)
{
	char 	arg1[MAX_INPUT_LENGTH];
	char 	arg2[MAX_INPUT_LENGTH];
	char 	arg3[MAX_INPUT_LENGTH];
...
void
do_pcquesting(CHAR_DATA * ch, char *argument)
{
	char 	arg[MAX_INPUT_LENGTH];
	CHAR_DATA *victim;
...
void
do_rppedit(CHAR_DATA * ch, char *argument)
{
	CHAR_DATA *victim;
	char 	arg1[MAX_INPUT_LENGTH];
	char 	arg2[MAX_INPUT_LENGTH];
	...
	if (arg1[0] == '\0') {
		send_to_char
		    ("'kay, genius. How do you plan to edit RPPs without specifying WHO?\n\r",
		    ch);
		return;
	}
	if ((victim = get_char_world(ch, arg1)) == NULL) {
	...
	if (arg2[0] == '+' || arg2[0] == '-') {
		...
		if ((arg2[0] == '+' && (value == 5))
			...
			if (arg2[0] == '+') {
				...
				ch_printf(victim,
				    "You have been awarded %d RPPs for your quality roleplaying.\n\r",
				    value);
				return;
			}
			if (arg2[0] == '-') {
...
void
do_dnsusers(CHAR_DATA * ch, char *argument)
{
	char 	buf[MAX_STRING_LENGTH];
	DESCRIPTOR_DATA *d;
	int 	count;
...
void
do_noscream(CHAR_DATA * ch, char *argument)
{
	CHAR_DATA *victim;
...
void
do_nognote(CHAR_DATA * ch, char *argument)
{
	CHAR_DATA *victim;
	char 	arg[MAX_INPUT_LENGTH];
	sh_int 	time;
	...
	if (!*arg || !str_cmp(arg, "hours"))
		...
		send_to_char
		    ("You may not nognote a person for more than 30 days at a time.\n\r",
		    ch);
		return;
	}
	tms = localtime(&current_time);
...
void
do_viewbuf(CHAR_DATA * ch, char *argument)
{
	CHAR_DATA *victim;
