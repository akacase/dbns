...
void
show_ships_to_char(SHIP_DATA * ship, CHAR_DATA * ch)
{
	SHIP_DATA *rship;
...
show_visible_affects_to_char(CHAR_DATA * victim, CHAR_DATA * ch)
{
	...
	if (IS_AFFECTED(victim, AFF_PERFECT)) {
		...
		act(AT_PURPLE,
		    ...
		    ch, NULL, victim, TO_CHAR);
		if (!IS_NPC(victim)
		    && victim->race == race_lookup("bio-android")) {
	...
	for (iWear = 0; iWear < MAX_WEAR; iWear++) {
		if ((obj = get_eq_char(victim, iWear)) != NULL
			...
			if ((!IS_NPC(victim)) && (victim->race > 0)
				...
				send_to_char(race_table[victim->race]->
				    where_name[iWear], ch);
			else
...
do_exits(CHAR_DATA * ch, char *argument)
{
	...
	for (pexit = ch->in_room->first_exit; pexit; pexit = pexit->next) {
		if (pexit->to_room && !IS_SET(pexit->exit_info, EX_CLOSED)
		    && (!IS_SET(pexit->exit_info, EX_WINDOW)
		    || IS_SET(pexit->exit_info, EX_ISDOOR))
		    && !IS_SET(pexit->exit_info, EX_HIDDEN)) {
			found = true;
...
find_whogr(DESCRIPTOR_DATA * d, struct whogr_s *first)
{
	...
	for (whogr = first; whogr; whogr = whogr->next)
		if (whogr->d == d)
			return (whogr);
		else if (whogr->follower
		    && (whogr_t = find_whogr(d, whogr->follower)))
			return (whogr_t);
...
indent_whogr(CHAR_DATA * looker, struct whogr_s *whogr, int ilev)
{
	for (; whogr; whogr = whogr->next) {
		if (whogr->follower) {
			int nlev = ilev;
			CHAR_DATA *wch =
			(whogr->d->original ? whogr->d->original : whogr->
			    d->character);
	...
	while (wc < dc)
		for (d = last_descriptor; d; d = d->prev) {
			...
			if (wch->leader && wch->leader != wch
			    && wch->leader->desc && !IS_NPC(wch->leader)
			    && !IS_IMMORTAL(wch) && !IS_IMMORTAL(wch->leader)
			    && (whogr_t =
			    find_whogr(wch->leader->desc, first_whogr))) {
				CREATE(whogr, struct whogr_s, 1);
				if (!whogr_t->l_follow)
...
do_who(CHAR_DATA * ch, char *argument)
{
	...
	for (;;) {
		...
		if (is_number(arg)) {
			...
			default:
				send_to_char
				    ("Only two rank numbers allowed.\n\r", ch);
				return;
			}
		} else {
			if (strlen(arg) < 3) {
				send_to_char
				    ("Arguments must be longer than that.\n\r",
				    ch);
				return;
			}
			/*
	...
	for (; d; whogr_p = whogr, whogr = (fGroup ? whogr->next : NULL),
		...
		if (fLeader && !(wch->pcdata->council &&
		    ...
		    !str_cmp(wch->pcdata->council->head2,
		    wch->name))
		    || (wch->pcdata->council->head
		    && !str_cmp(wch->pcdata->council->head,
		    wch->name))))
		...
		if (fShowHomepage
			...
			sprintf(last_name, "%s",
			    wch->pcdata->last_name ? wch->pcdata->
			    last_name : "");
		}
		...
		if (wch->pcdata->clan) {
			...
			if (pclan->clan_type == CLAN_ORDER) {
				...
				if (is_owner(wch, pclan))
					...
					if (str_cmp(get_clan_rank(wch), "")
					    && str_cmp(get_clan_rank(wch),
					    "None")) {
						strcat(clan_name,
						    get_clan_rank(wch));
						strcat(clan_name, " of ");
		...
		if (wch->pcdata->council) {
			...
			if (wch->pcdata->council->head2 == NULL) {
				if (!str_cmp
				    (wch->name, wch->pcdata->council->head))
		...
		sprintf(buf,
		    ...
		    incog_str,
		    (wch->desc
		    && wch->desc->connected) ? "&g[WRITING] " : "",
		    xIS_SET(wch->act, PLR_AFK) ? "&g[AFK] " : "",
		    xIS_SET(wch->affected_by, AFF_TAG) ? "&W[IT] " : "",
		    IS_SET(wch->pcdata->flags,
		    PCFLAG_DND) ? "&g[DND] " : "", xIS_SET(wch->act,
		    PLR_ATTACKER)
		    ? "&R(ATTACKER) " : "", xIS_SET(wch->act,
		    PLR_KILLER) ?
		    "&R(KILLER) " : "", xIS_SET(wch->act,
		    PLR_THIEF) ? "&R(THIEF) " :
	...
	if (arg[0] == '\0') {
		...
		for (d = first_descriptor; d; d = d->next)
			if ((d->connected == CON_PLAYING
			    ...
			    && (get_trust(ch) >= get_trust(victim)
			    || !IS_SET(victim->pcdata->flags, PCFLAG_DND))
			    ) {		/* if victim has the DND flag ch must
					 * outrank them */
				found = true;
				/*
				 * if ( CAN_PKILL( victim ) )
...
bool
gTrainSuccess(CHAR_DATA * ch, int stat, sh_int * tAbility)
{
	char buf[MAX_STRING_LENGTH];
	float xp_mod = 0;
	...
	*tAbility += increase;
	...
	sprintf(buf, "Your power level increases by %s points.",
	    num_punct(xp_gain));
...
/* TODO: GRAVTRAIN needs editing - case */
void
do_train(CHAR_DATA * ch, char *argument)
{
	...
	if (is_android(ch) || is_superandroid(ch)) {
		send_to_char
		    ("Androids can't learn anything by training like that.\n\r",
		    ch);
		return;
	}
	if (!xIS_SET((ch)->in_room->room_flags, ROOM_GRAV)) {
		send_to_char
		    ("Hmm... I don't see a grav training room around here...\n\r",
		    ch);
		return;
	}
	if (IS_IMMORTAL(ch)) {
	...
	if (ch->pcdata->tStat == 0) {
		if (ch->substate != SUB_TIMER_DO_ABORT) {
			...
			if (arg1[0] == '\0') {
				sprintf(buf,
				    "Syntax: gravtrain <attribute> <minutes>\n\r");
				send_to_char(buf, ch);
				pager_printf(ch,
				    "You have %d training points and %d extended training points\n\r",
			...
			if (trainTime <= 0) {
				pager_printf(ch,
				    "Last time I checked, time travel wasn't an option...\n\r");
				return;
			}
			if (trainTime >= 480) {
				pager_printf(ch,
				    "Training for more than 8 hours at a time isn't a good idea.\n\r");
				return;
			}
		}
	...
	if (*permTstat >= 100 && *tAbility >= 99 && ch->pcdata->xTrain < 1) {
		act(AT_RED, "Your $T is already at maximum.", ch, NULL, pOutput,
		    TO_CHAR);
		ch->pcdata->tStat = 0;
		ch->pcdata->tRounds = 0;
		return;
	...
	if (ch->train < 1 && ch->pcdata->xTrain < 1 && *tAbility >= 99) {
		pager_printf(ch,
		    "You don't have enough training points to work on your %s anymore.\n\r",
		    pOutput);
		ch->pcdata->tStat = 0;
		ch->pcdata->tRounds = 0;
		return;
	...
	case SUB_NONE:
		...
		gravLevel =
		    ((get_curr_str(ch) + get_curr_dex(ch) + get_curr_int(ch) +
		    get_curr_con(ch)) - 40) * 5;
		pager_printf(ch,
		    "&GYou turn the dial to %d times gravity and begin training your %s.\n\r",
		    gravLevel, pOutput);
		act(AT_SKILL, "$n starts training $s $T.", ch, NULL, pOutput,
		    TO_ROOM);
	...
	case 1:
		...
		if (ch->pcdata->tRounds <= 0) {
			ch->pcdata->tRounds = 0;
			act(AT_SKILL, "You stop training your $T.", ch, NULL,
			    pOutput, TO_CHAR);
			act(AT_SKILL, "$n stops training $s $T.", ch, NULL,
			    pOutput, TO_ROOM);
			break;
		}
		if (*tAbility >= 100) {
		...
		if (ch->hit <= minLF) {
			act(AT_SKILL,
			    "You almost pass out from training your $T to hard.",
			    ch, NULL, pOutput, TO_CHAR);
			act(AT_SKILL,
			    "$n almost passes out from training $s $T to hard.",
			    ch, NULL, pOutput, TO_ROOM);
			break;
		}
		if (number_range(1, 100) < 25) {
			...
			case 1:
				activity =
				    number_range(0, MAX_STR_TRAINING - 1);
				pager_printf(ch, strTrainUser[activity]);
				act(AT_SKILL, strTrainRoom[activity], ch, NULL,
				    pOutput, TO_ROOM);
				break;
			case 2:
				activity =
				    number_range(0, MAX_SPD_TRAINING - 1);
				pager_printf(ch, spdTrainUser[activity]);
				act(AT_SKILL, spdTrainRoom[activity], ch, NULL,
				    pOutput, TO_ROOM);
				break;
			case 3:
				activity =
				    number_range(0, MAX_INT_TRAINING - 1);
				pager_printf(ch, intTrainUser[activity]);
				act(AT_SKILL, intTrainRoom[activity], ch, NULL,
				    pOutput, TO_ROOM);
				break;
			case 4:
				activity =
				    number_range(0, MAX_CON_TRAINING - 1);
				pager_printf(ch, conTrainUser[activity]);
				act(AT_SKILL, conTrainRoom[activity], ch, NULL,
				    pOutput, TO_ROOM);
				break;
			}
			pager_printf(ch, "&w\n\r");
	...
	case 2:
		...
		if (ch->pcdata->tRounds <= 0) {
			ch->pcdata->tRounds = 0;
			act(AT_SKILL,
			    "You take a break and stop training your $T.", ch,
			    NULL, pOutput, TO_CHAR);
			act(AT_SKILL,
			    "$n takes a break and stops training $s $T.", ch,
			    NULL, pOutput, TO_ROOM);
			break;
		}
		if (*tAbility >= 100) {
			checkLearn = true;
	...
	case SUB_TIMER_DO_ABORT:
		ch->pcdata->tRounds = 0;
		act(AT_SKILL, "You stop training your $T.", ch, NULL, pOutput,
		    TO_CHAR);
		act(AT_SKILL, "$n stops training $s $T.", ch, NULL, pOutput,
		    TO_ROOM);
		break;
	}
	...
	if (*permTstat >= 100 && ch->pcdata->xTrain < 1 && *tAbility >= 99) {
		act(AT_SKILL, "Your $T has reached it's peak for now.", ch,
		    NULL, pOutput, TO_CHAR);
		*tAbility = 99;
		sysdata.outBytesFlag = LOGBOUTNORM;
		return;
	...
	if (checkLearn) {
		if (ch->train >= 1 && *permTstat < 100) {
			...
			*permTstat += 1;
			...
			act(AT_RED, "$n's $T increases!", ch, NULL, pOutput,
			    TO_ROOM);
			act(AT_SKILL,
			    "You take a break and stop training your $T.", ch,
			    NULL, pOutput, TO_CHAR);
			...
			*pAbility += 1;
			...
			act(AT_SKILL, "Your $T has reached its peak for now.",
			    ch, NULL, pOutput, TO_CHAR);
			*tAbility = 99;
		}
	}
...
do_practice(CHAR_DATA * ch, char *argument)
{
	...
	if (argument[0] == '\0') {
		...
		if (can_prac && ((sn == -1)
		    ...
		    skill_table[sn]->skill_level[ch->class]
		    && ch->level <
			skill_table[sn]->min_level[ch->class]))) {
			act(AT_TELL,
		...
		if (ch->race == race_lookup("bio-android")) {
			if (sn == gsn_semiperfect
			    || sn == gsn_perfect || sn == gsn_ultraperfect) {
				bioPrac = false;
		...
		if (ch->race == 6
		    && skill_table[sn]->skill_level[ch->class] >= 100000
		    && !bioPrac) {
	...
	if (arg[0] == '\0') {
		...
		if (IS_HC(ch) || IS_IMMORTAL(ch))
			ch_printf_color(ch, "%s",
			    !xIS_SET(ch->deaf,
			    CHANNEL_HC) ? " &G+HARD-CORE" :
			    " &g-hard-core");
		ch_printf_color(ch, "%s",
		    !xIS_SET(ch->deaf,
		    CHANNEL_ROLEPLAY) ? " &G+ROLEPLAY" :
		    " &g-roleplay");
		...
		if (!IS_NPC(ch) && ch->pcdata->clan) {
			if (ch->pcdata->clan->clan_type == CLAN_ORDER)
				send_to_char_color(!xIS_SET
				    (ch->deaf,
				    CHANNEL_ORDER) ? " &G+ORDER"
				    : " &g-order", ch);
		...
		if ((fClear) && (ClearAll)) {
			...
			if (bit == CHANNEL_FOS) {
				send_to_char
				    ("&RThe 'Freedom of Speech' channel is &WNOT &Rmoderated by administrators.\n\r",
				    ch);
				send_to_char
				    ("&RThis channel may contain vulgar language, obscene gestures, adult situations,\n\r",
				    ch);
				send_to_char
				    ("&Rfull frontal nudity, and graphic violence.  Then again...it may not.\n\r",
				    ch);
				send_to_char
				    ("&RYou &WMUST &Rbe at least 18 years of age to use or have this channel active.\n\r",
				    ch);
				send_to_char
				    ("&RIf you are not 18, please type '&Wchannels -fos&R' to turn off this channel.&w\n\r",
				    ch);
			}
...
do_config(CHAR_DATA * ch, char *argument)
{
	...
	if (arg[0] == '\0') {
		...
		set_char_color(AT_GREEN, ch);
		send_to_char
		    ("(use 'config +/- <keyword>' to toggle, or 'config nprompt/bprompt #' see 'help config')\n\r\n\r",
		    ch);
		set_char_color(AT_DGREEN, ch);
		...
		ch_printf(ch,
		    ...
		    PCFLAG_GAG) ? "[+] GAG" : "[-] gag",
		    xIS_SET(ch->act,
		    PLR_BRIEF) ? "[+] BRIEF" : "[-] brief",
		    xIS_SET(ch->act,
		    PLR_COMBINE) ? "[+] COMBINE" : "[-] combine",
		    xIS_SET(ch->act,
		    PLR_BLANK) ? "[+] BLANK" : "[-] blank",
		    xIS_SET(ch->act,
		    PLR_PROMPT) ? "[+] PROMPT" : "[-] prompt",
		...
		ch_printf(ch,
		    "%-12s   %-12s   %-12s   %-12s\n\r           %-12s",
		    xIS_SET(ch->act,
		    PLR_AUTOSAC) ? "[+] AUTOSAC" : "[-] autosac",
		    xIS_SET(ch->act,
		    PLR_AUTOGOLD) ? "[+] AUTOZENI" :
		    "[-] autozeni", xIS_SET(ch->act,
		    PLR_AUTOLOOT) ? "[+] AUTOLOOT"
		    : "[-] autoloot", xIS_SET(ch->act,
		    PLR_AUTOEXIT) ?
		    "[+] AUTOEXIT" : "[-] autoexit", xIS_SET(ch->act,
		    PLR_AUTO_COMPASS)
		    ? "[+] AUTOCOMPASS" : "[-] autocompass");
		...
		ch_printf(ch, "%-12s   %-12s",
		    ...
		    PCFLAG_NOSUMMON) ?
		    "[+] NOSUMMON" : "[-] nosummon", xIS_SET(ch->act,
		    PLR_NOFOLLOW)
		    ? "[+] NOFOLLOW" : "[-] nofollow");
 ...
 */
void
do_combat(CHAR_DATA * ch, char *argument)
{
	char arg[MAX_INPUT_LENGTH];
	...
	if (arg[0] == '\0') {
		...
		set_char_color(AT_GREEN, ch);
		send_to_char
		    ("(use 'combat +/- <keyword>' to toggle, see 'help combat')\n\r\n\r",
		    ch);
		set_char_color(AT_DGREEN, ch);
		send_to_char("\n\r\n\rDefenses:   \n\r", ch);
		set_char_color(AT_GREY, ch);
		...
		if (!str_prefix(arg + 1, "kiabsorb")) {
			...
			if (xIS_SET((ch)->affected_by, AFF_EXTREME)) {
				send_to_char
				    ("You cannot toggle heart while using Extreme.\n\r",
				    ch);
				return;
			}
			if (xIS_SET((ch)->affected_by, AFF_MYSTIC)) {
				send_to_char
				    ("You cannot toggle heart while using Mystic.\n\r",
				    ch);
				return;
			}
			if (arg[0] == '-') {
				if (xIS_SET((ch)->affected_by, AFF_HEART)) {
					...
					ch->pl = ch->heart_pl;
					send_to_char
					    ("Your power decreases as you suppress your inner strength.\n\r",
					    ch);
				}
			}
			bit = CMB_NO_HEART;
...
do_areas(CHAR_DATA * ch, char *argument)
{
	...
	if (arg[0] != '\0') {
		...
		if (arg[0] != '\0') {
			if (!is_number(arg)) {
				send_to_char
				    ("Area may only be followed by numbers.\n\r",
				    ch);
				return;
			}
			upper_bound = atoi(arg);
...
void
slistSort(int *array, const int size, int chRace)
{
	int pass;
	int j;
...
do_alist(CHAR_DATA * ch, char *argument)
{
	...
	for (sn = gsn_first_ability; sn < gsn_first_weapon; sn++) {
		...
		if (ch->race == 6 && ch->pcdata->learned[sn] <= 0
		    && skill_table[sn]->skill_level[ch->class] >= 100000
		    && !bioPrac)
			continue;
		...
		if (canUse) {
			if ((!mystic && !canMystic) || (mystic && canMystic))
				pager_printf_color(ch,
				    ...
				    ch->pcdata->
				    learned[snRace[i]],
				    skill_table[snRace[i]]->
				    skill_adept[ch->class]);
		} else {
			if ((!mystic && !canMystic) || (mystic && canMystic))
				pager_printf_color(ch,
	...
	for (i = 0; i < size; i++) {
		...
		if (canUse) {
			pager_printf_color(ch,
			    ...
			    skill_table[snRace[i]]->name,
			    num_punct_ld(skill_table[snRace[i]]->
...
do_whois(CHAR_DATA * ch, char *argument)
{
	...
	if (argument[0] == '\0') {
		send_to_pager
		    ("You must input the name of an online character.\n\r", ch);
		return;
	}
	strcat(buf, "0.");
	strcat(buf, argument);
	...
	if (victim->pcdata->spouse && victim->pcdata->spouse[0] != '\0')
		pager_printf(ch, " %s is married to %s.\n\r",
		    victim->sex == SEX_MALE ? "He" : victim->sex ==
		    SEX_FEMALE ? "She" : "It", victim->pcdata->spouse);
...
void
do_upgrade(CHAR_DATA * ch, char *argument)
{
	char buf[MAX_STRING_LENGTH];
	int sn;
	bool prac = false;
	...
	if (argument[0] == '\0') {
		...
		/* 'train' section */
			...
			if (!str_cmp(argument, "str")
			    || !str_cmp(argument, "strength")) {
				pAbility = &ch->perm_str;
				pOutput = "strength";
			} else if (!str_cmp(argument, "int")
			    || !str_cmp(argument, "intelligence")) {
				pAbility = &ch->perm_int;
				pOutput = "intelligence";
			} else if (!str_cmp(argument, "spd")
			    || !str_cmp(argument, "speed")) {
				pAbility = &ch->perm_dex;
				pOutput = "speed";
			} else if (!str_cmp(argument, "con")
			    || !str_cmp(argument, "constitution")) {
				pAbility = &ch->perm_con;
				pOutput = "constitution";
			} else {
			...
			if (ch->train < 1) {
				send_to_char
				    ("You don't have enough hardware upgrade points.\n\r",
				    ch);
				return;
			}
			ch->train -= 1;
...
void
do_nohelps(CHAR_DATA * ch, char *argument)
{
	CMDTYPE *command;
	AREA_DATA *tArea;
	char arg[MAX_STRING_LENGTH];
...
void
do_partner(CHAR_DATA * ch, char *argument)
{
	CHAR_DATA *victim;
	...
	if (argument[0] == '\0') {
		send_to_char
		    ("You have to pick someone as your sparring partner!\n\r",
		    ch);
		return;
	}
...
void
do_analyze(CHAR_DATA * ch, char *argument)
{
	OBJ_DATA *obj;
	AFFECT_DATA *paf;
	long double range = 0;
	...
	case ITEM_QUIVER:
		ch_printf(ch, "%s appears to %s.\n\r",
		    ...
		    150 ? "have a small to medium capacity" : obj->
		    value[0] <
		    300 ? "have a medium capacity" : obj->value[0] <
		    500 ? "have a medium to large capacity" : obj->
		    value[0] <
