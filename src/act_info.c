...
void do_exercise(CHAR_DATA *ch, char *argument)
{
	...
	if (ch->position != POS_RESTING) {
		...
		|| xIS_SET((ch)->affected_by, AFF_INTEXPUSHUPS) || xIS_SET((ch)->affected_by, AFF_INTEXSHADOWBOXING) || xIS_SET((ch)->affected_by, AFF_INTEXENDURING)) {
...
do_develop(CHAR_DATA * ch, char *argument)
{
	...
	if (!str_cmp(arg1, "makosen") && !str_cmp(arg2, "concentratedbeam")) {
		...
		if (((ch->makosenpower + ch->makoseneffic) >= 4)
			&& ((ch->concentrated_beampower + ch->concentrated_beameffic) >= 5)) {
