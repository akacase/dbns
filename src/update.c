...
void transform_oozaru(CHAR_DATA *ch) {
  ...
  act(AT_WHITE,
      ...
      NULL, NULL, TO_CHAR);
  act(AT_YELLOW,
      "You let out an earth shattering howl as you transform into Oozaru!",
      ch, NULL, NULL, TO_CHAR);
  act(AT_WHITE, "$n gazes up at the full moon and $e begins to change.",
      ch, NULL, NULL, TO_NOTVICT);
  act(AT_YELLOW,
      "$n lets out an earth shattering howl as $e transforms into Oozaru!",
      ch, NULL, NULL, TO_NOTVICT);
...
void untransform_oozaru(CHAR_DATA *ch) {
  act(AT_WHITE,
      "The full moon fades as the sun rises and you return to normal.",
      ch, NULL, NULL, TO_CHAR);
...
void transform_makeo(CHAR_DATA *ch) {
  ...
  act(AT_RED, "Sinister light from the Makeo Star shines down upon you.",
      ch, NULL, NULL, TO_CHAR);
  act(AT_RED,
      "You yell out as the evil energy radiating from the Makeo Star increases your power!",
      ch, NULL, NULL, TO_CHAR);
  act(AT_RED, "Sinister light from the Makeo Star shines down upon $n.",
      ch, NULL, NULL, TO_NOTVICT);
  act(AT_RED,
      "$n yells out as the evil energy radiating from the Makeo Star increases $s power!",
      ch, NULL, NULL, TO_NOTVICT);
...
void untransform_makeo(CHAR_DATA *ch) {
  ...
  act(AT_DGREY,
      "The Makeo Star leaves the Earth's orbit, returning you to normal.",
      ch, NULL, NULL, TO_CHAR);
...
void transform_golden_oozaru(CHAR_DATA *ch) {
  ...
  act(AT_WHITE,
      ...
      NULL, NULL, TO_CHAR);
  act(AT_YELLOW,
      "You let out an earth shattering howl as you transform into a Golden Oozaru!",
      ch, NULL, NULL, TO_CHAR);
  act(AT_WHITE, "$n gazes up at the full moon and $e begins to change.",
      ch, NULL, NULL, TO_NOTVICT);
  act(AT_YELLOW,
      "$n lets out an earth shattering howl as $e transforms into a Golden Oozaru!",
      ch, NULL, NULL, TO_NOTVICT);
...
int hit_gain(CHAR_DATA *ch) {
  ...
  if (xIS_SET((ch)->affected_by, AFF_INTEXPUSHUPS) || xIS_SET((ch)->affected_by, AFF_EXPUSHUPS) || xIS_SET((ch)->affected_by, AFF_EXENDURING) || xIS_SET((ch)->affected_by, AFF_INTEXENDURING) || xIS_SET((ch)->affected_by, AFF_INTEXSHADOWBOXING) || xIS_SET((ch)->affected_by, AFF_EXSHADOWBOXING) || xIS_SET((ch)->affected_by, AFF_POWERUPTRAIN)) {
    gain = 0;
  }
...
int mana_gain(CHAR_DATA *ch) {
  ...
  if (xIS_SET((ch)->affected_by, AFF_INTEXPUSHUPS) || xIS_SET((ch)->affected_by, AFF_EXPUSHUPS) || xIS_SET((ch)->affected_by, AFF_EXENDURING) || xIS_SET((ch)->affected_by, AFF_INTEXENDURING) || xIS_SET((ch)->affected_by, AFF_INTEXSHADOWBOXING) || xIS_SET((ch)->affected_by, AFF_EXSHADOWBOXING)) {
    gain = 0;
  }
...
void mobile_update(void) {
  ...
  for (ch = last_char; ch; ch = gch_prev) {
    ...
    /* Wander */
    if (!xIS_SET(ch->act, ACT_RUNNING) && !xIS_SET(ch->act, ACT_SENTINEL) && !xIS_SET(ch->act, ACT_PROTOTYPE) && (door = number_bits(5)) <= 9 && (pexit = get_exit(ch->in_room, door)) != NULL && pexit->to_room && !IS_SET(pexit->exit_info, EX_CLOSED) && !xIS_SET(pexit->to_room->room_flags, ROOM_NO_MOB) && !xIS_SET(pexit->to_room->room_flags, ROOM_DEATH) && (!xIS_SET(ch->act, ACT_STAY_AREA) || pexit->to_room->area == ch->in_room->area)) {
    ...
    /* Flee */
    if (ch->hit < ch->max_hit / 2 && (door = number_bits(4)) <= 9 && (pexit = get_exit(ch->in_room, door)) != NULL && pexit->to_room && !IS_SET(pexit->exit_info, EX_CLOSED) && !xIS_SET(pexit->to_room->room_flags, ROOM_NO_MOB)) {
      CHAR_DATA *rch;
      bool found;
  ...
  for (ch = last_char; ch; ch = gch_prev) {
    ...
    if (!IS_NPC(ch) && ch->level < LEVEL_IMMORTAL) {
      ...
      if (ch->pcdata->condition[COND_DRUNK] > 8)
        worsen_mental_state(ch,
                            ch->pcdata->condition[COND_DRUNK] / 8);
    ...
    if (!IS_NPC(ch)) {
      ...
      if (ch->pcdata->tail < 0)
        ...
        if (!skip) {
          act(AT_GREEN,
              "Your tail has grown back!", ch,
              NULL, NULL, TO_CHAR);
          act(AT_GREEN,
              "$n's tail has grown back!", ch,
              NULL, NULL, TO_ROOM);
          ch->pcdata->tail = 1;
        }
      }
    ...
    if (!IS_NPC(ch) && ch->pcdata->HBTCTimeLeft == 1 && xIS_SET(ch->in_room->room_flags, ROOM_TIME_CHAMBER) && !IS_IMMORTAL(ch)) {
      act(AT_ACTION,
          "Your time in the Hyperbolic Time Chamber is almost up!",
          ch, NULL, NULL, TO_CHAR);
    }
    if (!IS_NPC(ch) && ch->pcdata->HBTCTimeLeft <= 0 && xIS_SET(ch->in_room->room_flags, ROOM_TIME_CHAMBER) && !IS_IMMORTAL(ch)) {
    ...
    if (!IS_NPC(ch))
      mccp_interest(ch);
    if (!IS_NPC(ch) && !IS_IMMORTAL(ch) && (ch->in_room->vnum == 6 || ch->in_room->vnum == 8) && ch->pcdata->release_date == 0) {
      ROOM_INDEX_DATA *location;
    ...
    if (number_percent() < 25 && !IS_IMMORTAL(ch)) {
      if (IS_AFFECTED(ch, AFF_SSJ) && !IS_AFFECTED(ch, AFF_SSJ2) && !IS_AFFECTED(ch, AFF_SSJ3) && !IS_AFFECTED(ch, AFF_SSJ4)) {
        act(AT_YELLOW,
            "The yellow aura around your body hums softly.",
            ch, NULL, NULL, TO_CHAR);
        act(AT_YELLOW,
            "The yellow aura around $n's body hums softly.",
            ch, NULL, NULL, TO_NOTVICT);
      }
      if (IS_AFFECTED(ch, AFF_SSJ) && IS_AFFECTED(ch, AFF_SSJ2) && !IS_AFFECTED(ch, AFF_SSJ3) && !IS_AFFECTED(ch, AFF_SSJ4)) {
        switch (number_range(1, 3)) {
          case 1:
            act(AT_BLUE,
                "Electricity crackles around you.",
                ch, NULL, NULL, TO_CHAR);
            act(AT_BLUE,
                "Electricity crackles around $n.",
                ch, NULL, NULL, TO_NOTVICT);
            break;
          case 2:
            act(AT_BLUE,
                "Electricity arcs around you.", ch,
                NULL, NULL, TO_CHAR);
            act(AT_BLUE,
                "Electricity arcs around $n.", ch,
                NULL, NULL, TO_NOTVICT);
            break;
          case 3:
            act(AT_BLUE,
                "Lightning crackles around you, flowing through your yellow aura.",
                ch, NULL, NULL, TO_CHAR);
            act(AT_BLUE,
                "Lightning crackles around $n, flowing through $s yellow aura.",
                ch, NULL, NULL, TO_NOTVICT);
            break;
        }
      }
      ...
      if (IS_AFFECTED(ch, AFF_SSJ) && IS_AFFECTED(ch, AFF_SSJ2) && IS_AFFECTED(ch, AFF_SSJ3) && !IS_AFFECTED(ch, AFF_SSJ4)) {
        switch (number_range(1, 3)) {
          ...
          case 3:
            act(AT_YELLOW,
                "Your mane of hair shifts slightly, your aura pulsating with golden energy.",
                ch, NULL, NULL, TO_CHAR);
            act(AT_YELLOW,
                "$n's mane of hair shifts slightly, $s aura pulsating with golden energy.",
                ch, NULL, NULL, TO_NOTVICT);
            break;
        }
      }
      ...
      if (IS_AFFECTED(ch, AFF_SNAMEK)) {
        act(AT_GREEN,
            "The green flames around you flare up brightly.",
            ch, NULL, NULL, TO_CHAR);
        act(AT_GREEN,
            "The green flames around $n flare up brightly.",
            ch, NULL, NULL, TO_NOTVICT);
      }
      if (IS_AFFECTED(ch, AFF_ICER4) || IS_AFFECTED(ch, AFF_ICER5)) {
        act(AT_BLUE,
            "Your icy aura sends a freezing cold breeze through the room.",
            ch, NULL, NULL, TO_CHAR);
        act(AT_BLUE,
      ...
      if (IS_AFFECTED(ch, AFF_KAIOKEN)) {
        act(AT_RED,
            ...
            ch, NULL, NULL, TO_CHAR);
        act(AT_RED,
            "A hum fills the room as the mystical flames around $n tingle your senses.",
            ch, NULL, NULL, TO_NOTVICT);
      }
      if (IS_AFFECTED(ch, AFF_HEART)) {
        act(AT_RED,
            "The pounding of your heartbeat echoes throughout the room.",
            ch, NULL, NULL, TO_CHAR);
        act(AT_RED,
            "The pounding of $n's heartbeat echoes throughout the room.",
            ch, NULL, NULL, TO_NOTVICT);
      }
      if (IS_AFFECTED(ch, AFF_ELECTRICSHIELD)) {
        act(AT_BLUE,
            "The shield of electricity around you crackles and hums.",
            ch, NULL, NULL, TO_CHAR);
        act(AT_BLUE,
            "The shield of electricity around $n crackles and hums.",
            ch, NULL, NULL, TO_NOTVICT);
      }
      if (IS_AFFECTED(ch, AFF_HYPER)) {
        act(AT_WHITE,
            "You flex your bulging muscles, veins throbbing as pure energy courses through your body.",
            ch, NULL, NULL, TO_CHAR);
        act(AT_WHITE,
            "$n flexes $s bulging muscles, veins throbbing as pure energy courses through $s body.",
            ch, NULL, NULL, TO_NOTVICT);
      }
      if (IS_AFFECTED(ch, AFF_EXTREME)) {
        act(AT_WHITE,
            "The aura around you crackles with bolts of trancendent power.",
            ch, NULL, NULL, TO_CHAR);
        act(AT_WHITE,
            "The aura around $n crackles with bolts of trancendent power.",
            ch, NULL, NULL, TO_NOTVICT);
      }
      if (IS_AFFECTED(ch, AFF_SEMIPERFECT)) {
        act(AT_PURPLE,
            "Your aura flashes purple, black lightning crackling before the energy melds back together.",
            ch, NULL, NULL, TO_CHAR);
        act(AT_PURPLE,
            "$n's aura flashes purple, black lightning crackling before the energy melds back together.",
            ch, NULL, NULL, TO_NOTVICT);
      }
      if (IS_AFFECTED(ch, AFF_PERFECT)) {
        act(AT_DGREY,
            "The energy around you flickers and dances, the wailing of dead souls filling the air.",
            ch, NULL, NULL, TO_CHAR);
        act(AT_DGREY,
            "The energy around $n flickers and dances, the wailing of dead souls filling the air.",
            ch, NULL, NULL, TO_NOTVICT);
      }
      if (IS_AFFECTED(ch, AFF_ULTRAPERFECT)) {
        act(AT_DGREY,
            "The black energy around you crackles through your stolen aura, drawing it momentarily "
            "closer to your form.",
            ch, NULL, NULL,
            TO_CHAR);
        act(AT_DGREY,
            "The black energy around $n crackles through $s stolen aura, drawing it momentarily "
            "closer to $s form.",
            ch, NULL, NULL,
            TO_NOTVICT);
      }
    ...
    if (!char_died(ch)) {
        ...
        for (paf = ch->first_affect; paf;
          ...
          if (paf->location ==
            ...
            if (IS_VALID_SN(paf->modifier) && (skill = skill_table[paf->modifier]) != NULL && skill->type == SKILL_SPELL) {
              if ((*skill->spell_fun)(paf->modifier,
                                      ch->level,
                                      ch,
                                      ch) ==
                      rCHAR_DIED ||
      ...
      if (ch->mental_state <= -30)
        switch ((abs(ch->mental_state) + 5) / 10) {
          ...
          case 8:
            if (ch->position > POS_SLEEPING) {
              if (ch->position < POS_SITTING && (number_percent() + 30) <
                                                    abs(ch->mental_state))
                do_sleep(ch, "");
              else
      ...
      if (ch == ch_save && IS_SET(sysdata.save_flags, SV_AUTO) && ++save_count < 10) /* save max of 10 per
							 * tick */
        save_char_obj(ch);
    }
  }
...
void obj_update(void) {
  ...
  for (obj = last_object; obj; obj = gobj_prev) {
    ...
    switch (obj->item_type) {
      ...
      case ITEM_CORPSE_PC:
        message =
            "$p is sucked into a swirling vortex of colors...";
        AT_TEMP = AT_MAGIC;
        break;
      case ITEM_COOK:
      case ITEM_FOOD:
  ...
  for (ch = first_char; ch; ch = ch_next) {
    ...
    if (IS_NPC(ch)) {
      ...
      if (xIS_SET(ch->act, ACT_RUNNING)) {
        ...
        if (!xIS_SET(ch->act, ACT_SENTINEL) && !xIS_SET(ch->act, ACT_PROTOTYPE) && (door = number_bits(4)) <= 9 && (pexit = get_exit(ch->in_room, door)) != NULL && pexit->to_room && !IS_SET(pexit->exit_info, EX_CLOSED) && !xIS_SET(pexit->to_room->room_flags, ROOM_NO_MOB) && !xIS_SET(pexit->to_room->room_flags, ROOM_DEATH) && (!xIS_SET(ch->act, ACT_STAY_AREA) || pexit->to_room->area == ch->in_room->area)) {
          retcode = move_char(ch, pexit, 0);
          if (char_died(ch))
            continue;
          if (retcode != rNONE || xIS_SET(ch->act, ACT_SENTINEL) || ch->position < POS_STANDING)
      ...
      if ((ch->in_room && ch->in_room->sector_type == SECT_UNDERWATER) || (ch->in_room && ch->in_room->sector_type ==
                                                                                              SECT_OCEANFLOOR)) {
        if (!IS_AFFECTED(ch, AFF_AQUA_BREATH)) {
          if (ch->level < LEVEL_IMMORTAL && !is_icer(ch) && !is_android(ch) && !is_superandroid(ch) && !wearing_sentient_chip(ch) && !is_bio(ch)) {
            int dam;
      ...
      if ((ch->in_room && ch->in_room->sector_type == SECT_SPACE)) {
        if (!is_icer(ch) && !is_android(ch) && !is_bio(ch) && !is_superandroid(ch) && !wearing_sentient_chip(ch)) {
      ...
      if (ch->in_room && ((ch->in_room->sector_type == SECT_WATER_NOSWIM) || (ch->in_room->sector_type ==
                                                                              SECT_WATER_SWIM))) {
      ...
      if (!ch->desc) {
        ...
        for (wch = ch->in_room->first_person; wch;
          ...
          if ((!xIS_SET(wch->act, ACT_AGGRESSIVE) && !xIS_SET(wch->act,
                                                              ACT_META_AGGR)) ||
  ...
  for (d = first_descriptor; d; d = dnext) {
    ...
    for (ch = wch->in_room->first_person; ch; ch = ch_next) {
      ...
      for (vch = wch->in_room->first_person; vch;
        ...
        if ((!IS_NPC(vch) || xIS_SET(ch->act, ACT_META_AGGR) || xIS_SET(vch->act, ACT_ANNOYING)) && vch->level < LEVEL_IMMORTAL && (!xIS_SET(ch->act, ACT_WIMPY) || !IS_AWAKE(vch)) && can_see(ch, vch)) {
      ...
      if (IS_NPC(ch) && xIS_SET(ch->attacks, ATCK_BACKSTAB)) {
        ...
        if (!ch->mount && (obj = get_eq_char(ch, WEAR_WIELD)) != NULL && (obj->value[3] == 11 || obj->value[3] == 2) && !victim->fighting && victim->hit >= victim->max_hit) {
          ...
          WAIT_STATE(ch,
                     skill_table[gsn_backstab]->beats);
...
void update_alliances(void) {
  ...
  for (alliance = first_alliance; alliance; alliance = alliance->next) {
    ...
    if (alliance->changeTimer <= 0) {
      ...
      valliance =
          get_alliance(alliance->vclan, alliance->clan);
...
void bump_que() {
  ...
  for (i = 0; i < AUCTION_QUE; i++) {
    ...
    auction->queued_starting_bid[i] =
        auction->queued_starting_bid[i + 1];
  }
  auction->queued_obj[2] = auction->queued_obj[3];
  auction->queued_char[2] = auction->queued_char[3];
  auction->queued_starting_bid[2] = auction->queued_starting_bid[3];
  auction->queued_obj[3] = NULL;
  auction->queued_char[3] = NULL;
  auction->queued_starting_bid[3] = 0;
