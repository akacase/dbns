...
void char_update(void)
{
	...
	for (ch = last_char; ch; ch = gch_prev) {
		...
		if (number_percent() < 25 && !IS_IMMORTAL(ch)) {
			...
			if (IS_AFFECTED(ch, AFF_ICER4)
			    || IS_AFFECTED(ch, AFF_ICER5)) {
				act(AT_BLUE,
			...
			if (IS_AFFECTED(ch, AFF_ULTRAPERFECT)) {
				act(AT_DGREY,
				    ...
				    "closer to your form.", ch, NULL, NULL,
				    TO_CHAR);
				act(AT_DGREY,
...
void aggr_update(void)
{
	...
	for (d = first_descriptor; d; d = dnext) {
		...
		if (char_died(wch)
		    || IS_NPC(wch)
		    || wch->level >= LEVEL_IMMORTAL || !wch->in_room)
			continue;
...
char *get_moon_echo(WEATHER_DATA * weath)
{
	...
	case 29:
		{
			return
			    ("&YThe full moon shines brightly down on you.\n\r");
		}
	}
