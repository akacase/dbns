...
bool check_pilot(CHAR_DATA *ch, SHIP_DATA *ship) {
  if (!str_cmp(ch->name, ship->owner) || !str_cmp(ch->name, ship->pilot) || !str_cmp(ch->name, ship->copilot) || !str_cmp("Public", ship->owner))
    return (true);
...
ship_from_cockpit(int vnum) {
  ...
  for (ship = first_ship; ship; ship = ship->next)
    if (vnum == ship->cockpit || vnum == ship->turret1 || vnum == ship->turret2 || vnum == ship->pilotseat || vnum == ship->coseat || vnum == ship->navseat || vnum == ship->gunseat || vnum == ship->engineroom)
      return (ship);
  return (NULL);
}
...
void echo_to_cockpit(int color, SHIP_DATA *ship, char *argument) {
  ...
  for (room = ship->firstroom; room <= ship->lastroom; room++) {
    if (room == ship->cockpit || room == ship->navseat || room == ship->pilotseat || room == ship->coseat || room == ship->gunseat || room == ship->engineroom || room == ship->turret1 || room == ship->turret2)
...
bool load_ship_file(char *shipfile) {
  ...
  if (!(found))
    ...
    } else if ((pRoomIndex = get_room_index(ship->lastdoc)) != NULL && ship->class != CAPITAL_SHIP && ship->class != SHIP_PLATFORM) {
    ...
    if ((pRoomIndex = get_room_index(ship->lastdoc)) == NULL && ship->class != CAPITAL_SHIP && ship->class != SHIP_PLATFORM) {
...
void landship(SHIP_DATA *ship, char *arg) {
  ...
  if (target != ship && target != NULL && target->bayopen && (ship->class != MIDSIZE_SHIP || target->class != MIDSIZE_SHIP))
...
void move_ships() {
  ...
  for (missile = first_missile; missile; missile = m_next) {
    ...
    if (target->starsystem && target->starsystem == missile->starsystem) {
      ...
      if (abs(missile->mx) - fabsf(target->vx) <= 20 && abs(missile->mx) - fabsf(target->vx) >= -20 && abs(missile->my) - fabsf(target->vy) <= 20 && abs(missile->my) - fabsf(target->vy) >= -20 && abs(missile->mz) - fabsf(target->vz) <= 20 && abs(missile->mz) - fabsf(target->vz) >= -20) {
      ...
      if (ship->starsystem) {
        if (ship->target0 && ship->statet0 != LASER_DAMAGED) {
          ...
          for (shots = 0; shots <= ship->lasers; shots++) {
            if (ship->shipstate != SHIP_HYPERSPACE && ship->energy > 25 && ship->target0->starsystem == ship->starsystem && fabsf(target->vx - ship->vx) <= 1000 && fabsf(target->vy - ship->vy) <= 1000 && fabsf(target->vz - ship->vz) <= 1000 && ship->statet0 < ship->lasers) {
    ...
    if (autofly(ship)) {
      if (ship->starsystem) {
        if (ship->target0) {
          ...
          if (ship->shipstate != SHIP_HYPERSPACE && ship->energy > 25 && ship->missilestate == MISSILE_READY && ship->target0->starsystem == ship->starsystem && fabsf(target->vx - ship->vx) <= 1200 && fabsf(target->vy - ship->vy) <= 1200 && fabsf(target->vz - ship->vz) <= 1200 && ship->missiles > 0) {
            if (ship->class > 1 || is_facing(ship, target)) {
...
void do_showship(CHAR_DATA *ch, char *argument) {
  ...
  pager_printf_color(ch, "%s %s : %s\n\rFilename: %s\n\r",
                     ship->type == SHIP_REPUBLIC ? "New Republic" : (ship->type == SHIP_IMPERIAL ? "Imperial" : (ship->type == SHIP_CIVILIAN ? "Civilian" : "Mob")),
                     ship->class == FIGHTER_SHIP ? "Starfighter" : (ship->class == MIDSIZE_SHIP ? "Midship" : (ship->class == CAPITAL_SHIP ? "Capital Ship" : (ship->class == SHIP_PLATFORM ? "Platform" : (ship->class == CLOUD_CAR ? "Cloudcar" : (ship->class == OCEAN_SHIP ? "Boat" : (ship->class == LAND_SPEEDER ? "Speeder" : (ship->class == WHEELED ? "Wheeled Transport" : (ship->class == LAND_CRAWLER ? "Crawler" : (ship->class == WALKER ? "Walker" : "Unknown"))))))))),
                     ship->name,
                     ship->filename);
...
void do_shipinfo(CHAR_DATA *ch, char *argument) {
  ...
  pager_printf_color(ch, "&Y%s %s : %s\n\r&B",
                     target->type == SHIP_REPUBLIC ? "New Republic" : (target->type == SHIP_IMPERIAL ? "Imperial" : "Civilian"),
                     target->class == FIGHTER_SHIP ? "Starfighter" : (target->class == MIDSIZE_SHIP ? "Midtarget" : (target->class == CAPITAL_SHIP ? "Capital Ship" : (ship->class == SHIP_PLATFORM ? "Platform" : (ship->class == CLOUD_CAR ? "Cloudcar" : (ship->class == OCEAN_SHIP ? "Boat" : (ship->class == LAND_SPEEDER ? "Speeder" : (ship->class == WHEELED ? "Wheeled Transport" : (ship->class == LAND_CRAWLER ? "Crawler" : (ship->class == WALKER ? "Walker" : "Unknown"))))))))),
                     target->name,
                     target->filename);
...
void do_hyperspace(CHAR_DATA *ch, char *argument) {
  ...
  for (eShip = ship->starsystem->first_ship; eShip; eShip = eShip->next_in_starsystem) {
    ...
    if (fabsf(eShip->vx - ship->vx) < 500 && fabsf(eShip->vy - ship->vy) < 500 && fabsf(eShip->vz - ship->vz) < 500) {
    ...
    for (starsystem = first_starsystem; starsystem; starsystem = starsystem->next) {
      ...
      pager_printf_color(ch, "%-30s %d\n\r", starsystem->name,
                         (abs(starsystem->xpos - ship->starsystem->xpos) +
                          abs(starsystem->ypos - ship->starsystem->ypos)) /
                             2);
      count++;
    }
    if (!count) {
...
drive_ship(CHAR_DATA *ch, SHIP_DATA *ship, EXIT_DATA *pexit, int fall) {
  ...
  if (!fall) {
    if (xIS_SET(to_room->room_flags, ROOM_INDOORS) || xIS_SET(to_room->room_flags, ROOM_SPACECRAFT) || to_room->sector_type == SECT_INSIDE) {
    ...
    if (in_room->sector_type == SECT_WATER_NOSWIM || to_room->sector_type == SECT_WATER_NOSWIM || to_room->sector_type == SECT_WATER_SWIM || to_room->sector_type == SECT_UNDERWATER || to_room->sector_type == SECT_OCEANFLOOR) {
...
bool is_dock(ROOM_INDEX_DATA *room) {
  ...
  for (starsystem = first_starsystem; starsystem; starsystem = starsystem->next) {
    if (vnum == starsystem->doc1a || vnum == starsystem->doc1b || vnum == starsystem->doc1c || vnum == starsystem->doc2a || vnum == starsystem->doc2b || vnum == starsystem->doc2c || vnum == starsystem->doc3a || vnum == starsystem->doc3b || vnum == starsystem->doc3c)
      return (true);
  }
  return (false);
}
