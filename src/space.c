...
SHIP_DATA *
ship_in_room(ROOM_INDEX_DATA * room, char *name)
{
	SHIP_DATA *ship;
...
bool 
check_pilot(CHAR_DATA * ch, SHIP_DATA * ship)
{
...
/* ARGSUSED */
bool 
is_rental(CHAR_DATA * ch, SHIP_DATA * ship)
{
...
bool 
rent_ship(CHAR_DATA * ch, SHIP_DATA * ship)
{
...
bool 
land_bus(SHIP_DATA * ship, int destination)
{
 ...
 */
SHIP_DATA *
get_ship_here(char *name, SPACE_DATA * starsystem)
{
	SHIP_DATA *ship;
...
void 
ship_from_starsystem(SHIP_DATA * ship, SPACE_DATA * starsystem)
{
...
void 
echo_to_cockpit(int color, SHIP_DATA * ship, char *argument)
{
	int 	room;
...
void 
echo_to_ship(int color, SHIP_DATA * ship, char *argument)
{
	int 	room;
...
void 
echo_to_room_dnr(int ecolor, ROOM_INDEX_DATA * room, char *argument)
{
	CHAR_DATA *vic;
...
bool 
is_facing(SHIP_DATA * ship, SHIP_DATA * target)
{
 ...
 */
SPACE_DATA *
starsystem_from_name(char *name)
{
 ...
 */
void 
save_starsystem(SPACE_DATA * starsystem)
{
	FILE   *fp;
	char 	filename[256];
	char 	buf[MAX_STRING_LENGTH];
...
void 
fread_starsystem(SPACE_DATA * starsystem, FILE *fp)
{
	char 	buf[MAX_STRING_LENGTH];
	char   *word;
	bool 	fMatch;
...
bool 
load_starsystem(char *starsystemfile)
{
	char 	filename[256];
	SPACE_DATA *starsystem;
	FILE   *fp;
...
load_space()
{
	...
	sprintf(starsystemlist, "%s", SPACE_LIST);
...
void 
fread_ship(SHIP_DATA * ship, FILE *fp)
{
	char 	buf[MAX_STRING_LENGTH];
	char   *word;
	bool 	fMatch;
		...
		case 'E':
			...
			if (!str_cmp(word, "End")) {
				...
				if (ship->missiletype == 1) {
					ship->torpedos = ship->missiles;	/* for back
										 * compatability */
					ship->missiles = 0;
				}
				ship->starsystem = NULL;
...
bool 
ship_to_room(SHIP_DATA * ship, int vnum)
{
...
void 
ship_to_starsystem(SHIP_DATA * ship, SPACE_DATA * starsystem)
{
	if (starsystem == NULL)
		return;
...
void 
landship(SHIP_DATA * ship, char *arg)
{
	SHIP_DATA *target;
	char 	buf[MAX_STRING_LENGTH];
	int 	destination = 0;
...
void 
destroy_ship(SHIP_DATA * ship, CHAR_DATA * ch)
{
	char 	buf[MAX_STRING_LENGTH];
	int 	roomnum;
	ROOM_INDEX_DATA *room;
...
void 
new_missile(SHIP_DATA * ship, SHIP_DATA * target, CHAR_DATA * ch, int missiletype)
{
	SPACE_DATA *starsystem;
...
move_ships()
{
	...
	for (missile = first_missile; missile; missile = m_next) {
		...
		if (target->starsystem && target->starsystem == missile->starsystem) {
			...
			if (abs(missile->mx) - fabsf(target->vx) <= 20 && abs(missile->mx) - fabsf(target->vx) >= -20
			    && abs(missile->my) - fabsf(target->vy) <= 20 && abs(missile->my) - fabsf(target->vy) >= -20
			    && abs(missile->mz) - fabsf(target->vz) <= 20 && abs(missile->mz) - fabsf(target->vz) >= -20) {
	...
	for (ship = first_ship; ship; ship = ship->next) {
		...
		if (ship->starsystem->star1 && strcmp(ship->starsystem->star1, "") &&
		    fabsf(ship->vx - ship->starsystem->s1x) < 10 &&
		    fabsf(ship->vy - ship->starsystem->s1y) < 10 &&
		    fabsf(ship->vz - ship->starsystem->s1z) < 10) {
		...
		if (ship->starsystem->star2 && strcmp(ship->starsystem->star2, "") &&
		    fabsf(ship->vx - ship->starsystem->s2x) < 10 &&
		    fabsf(ship->vy - ship->starsystem->s2y) < 10 &&
		    fabsf(ship->vz - ship->starsystem->s2z) < 10) {
			echo_to_cockpit(AT_BLOOD + AT_BLINK, ship, "You fly directly into the sun.");
		...
		if (ship->currspeed > 0) {
			if (ship->starsystem->planet1 && strcmp(ship->starsystem->planet1, "") &&
			    fabsf(ship->vx - ship->starsystem->p1x) < 10 &&
			    fabsf(ship->vy - ship->starsystem->p1y) < 10 &&
			    fabsf(ship->vz - ship->starsystem->p1z) < 10) {
			...
			if (ship->starsystem->planet2 && strcmp(ship->starsystem->planet2, "") &&
			    fabsf(ship->vx - ship->starsystem->p2x) < 10 &&
			    fabsf(ship->vy - ship->starsystem->p2y) < 10 &&
			    fabsf(ship->vz - ship->starsystem->p2z) < 10) {
			...
			if (ship->starsystem->planet3 && strcmp(ship->starsystem->planet3, "") &&
			    fabsf(ship->vx - ship->starsystem->p3x) < 10 &&
			    fabsf(ship->vy - ship->starsystem->p3y) < 10 &&
			    fabsf(ship->vz - ship->starsystem->p3z) < 10) {
				sprintf(buf, "You begin orbitting %s.", ship->starsystem->planet2);
				echo_to_cockpit(AT_YELLOW, ship, buf);
				sprintf(buf, "%s begins orbiting %s.", ship->name, ship->starsystem->planet2);
				echo_to_system(AT_ORANGE, ship, buf, NULL);
				...
				if (ship->target0 && ship->statet0 != LASER_DAMAGED) {
					...
					for (shots = 0; shots <= ship->lasers; shots++) {
						if (ship->shipstate != SHIP_HYPERSPACE && ship->energy > 25
						    ...
						    && fabsf(target->vx - ship->vx) <= 1000
						    && fabsf(target->vy - ship->vy) <= 1000
						    && fabsf(target->vz - ship->vz) <= 1000
							...
							if (ship->class > 1 || is_facing(ship, target)) {
								...
								chance -= (fabsf(target->vx - ship->vx) / 70);
								chance -= (fabsf(target->vy - ship->vy) / 70);
								chance -= (fabsf(target->vz - ship->vz) / 70);
...
update_space()
{
	...
	for (ship = first_ship; ship; ship = ship->next) {
		...
		/*
		 * following was originaly to fix ships that lost their
		 * pilot in the middle of a manuever and are stuck in a busy
		 * state but now used for timed manouevers such as turning
		 */
		...
		if (ship->starsystem) {
			...
			for (target = ship->starsystem->first_ship; target; target = target->next_in_starsystem) {
				...
				if (target != ship &&
				    fabsf(ship->vx - target->vx) < target_too_close &&
				    fabsf(ship->vy - target->vy) < target_too_close &&
				    fabsf(ship->vz - target->vz) < target_too_close) {
			...
			if (ship->starsystem->star1 && strcmp(ship->starsystem->star1, "") &&
			    fabsf(ship->vx - ship->starsystem->s1x) < too_close &&
			    fabsf(ship->vy - ship->starsystem->s1y) < too_close &&
			    fabsf(ship->vz - ship->starsystem->s1z) < too_close) {
				sprintf(buf, "Proximity alert: %s  %d %d %d", ship->starsystem->star1,
				    ship->starsystem->s1x, ship->starsystem->s1y, ship->starsystem->s1z);
				echo_to_room(AT_RED, get_room_index(ship->pilotseat), buf);
			}
			if (ship->starsystem->star2 && strcmp(ship->starsystem->star2, "") &&
			    fabsf(ship->vx - ship->starsystem->s2x) < too_close &&
			    fabsf(ship->vy - ship->starsystem->s2y) < too_close &&
			    fabsf(ship->vz - ship->starsystem->s2z) < too_close) {
				sprintf(buf, "Proximity alert: %s  %d %d %d", ship->starsystem->star2,
				    ship->starsystem->s2x, ship->starsystem->s2y, ship->starsystem->s2z);
				echo_to_room(AT_RED, get_room_index(ship->pilotseat), buf);
			}
			if (ship->starsystem->planet1 && strcmp(ship->starsystem->planet1, "") &&
			    fabsf(ship->vx - ship->starsystem->p1x) < too_close &&
			    fabsf(ship->vy - ship->starsystem->p1y) < too_close &&
			    fabsf(ship->vz - ship->starsystem->p1z) < too_close) {
				sprintf(buf, "Proximity alert: %s  %d %d %d", ship->starsystem->planet1,
				    ship->starsystem->p1x, ship->starsystem->p1y, ship->starsystem->p1z);
				echo_to_room(AT_RED, get_room_index(ship->pilotseat), buf);
			}
			if (ship->starsystem->planet2 && strcmp(ship->starsystem->planet2, "") &&
			    fabsf(ship->vx - ship->starsystem->p2x) < too_close &&
			    fabsf(ship->vy - ship->starsystem->p2y) < too_close &&
			    fabsf(ship->vz - ship->starsystem->p2z) < too_close) {
				sprintf(buf, "Proximity alert: %s  %d %d %d", ship->starsystem->planet2,
				    ship->starsystem->p2x, ship->starsystem->p2y, ship->starsystem->p2z);
				echo_to_room(AT_RED, get_room_index(ship->pilotseat), buf);
			}
			if (ship->starsystem->planet3 && strcmp(ship->starsystem->planet3, "") &&
			    fabsf(ship->vx - ship->starsystem->p3x) < too_close &&
			    fabsf(ship->vy - ship->starsystem->p3y) < too_close &&
			    fabsf(ship->vz - ship->starsystem->p3z) < too_close) {
			...
			if (ship->starsystem) {
				if (ship->target0) {
					...
					if (ship->shipstate != SHIP_HYPERSPACE && ship->energy > 25
					    ...
					    && fabsf(target->vx - ship->vx) <= 1200
					    && fabsf(target->vy - ship->vy) <= 1200
					    && fabsf(target->vz - ship->vz) <= 1200
					    && ship->missiles > 0) {
						if (ship->class > 1 || is_facing(ship, target)) {
							chance -= target->manuever / 5;
							chance -= target->currspeed / 20;
							chance += target->class * target->class * 25;
							chance -= (fabsf(target->vx - ship->vx) / 100);
							chance -= (fabsf(target->vy - ship->vy) / 100);
							chance -= (fabsf(target->vz - ship->vz) / 100);
							chance += (30);
							chance = URANGE(10, chance, 90);
...
void 
damage_ship_ch(SHIP_DATA * ship, int min, int max, CHAR_DATA * ch)
{
...
void 
damage_ship(SHIP_DATA * ship, int min, int max)
{
	int 	damage, shield_dmg;
...
void 
extract_missile(MISSILE_DATA * missile)
{
	SPACE_DATA *starsystem;
...
void 
do_setstarsystem(CHAR_DATA * ch, char *argument)
{
...
void 
showstarsystem(CHAR_DATA * ch, SPACE_DATA * starsystem)
{
...
void 
do_showstarsystem(CHAR_DATA * ch, char *argument)
{
	SPACE_DATA *starsystem;
...
void 
do_makestarsystem(CHAR_DATA * ch, char *argument)
{
...
/* ARGSUSED */
void 
do_starsystems(CHAR_DATA * ch, char *argument)
{
	SPACE_DATA *starsystem;
	int 	count = 0;
...
void 
do_resetship(CHAR_DATA * ch, char *argument)
{
	SHIP_DATA *ship;
...
void 
do_setship(CHAR_DATA * ch, char *argument)
{
	char 	arg1[MAX_INPUT_LENGTH];
	char 	arg2[MAX_INPUT_LENGTH];
	SHIP_DATA *ship;
...
void 
do_showship(CHAR_DATA * ch, char *argument)
{
	SHIP_DATA *ship;
...
void 
do_makeship(CHAR_DATA * ch, char *argument)
{
	SHIP_DATA *ship;
	char 	arg[MAX_INPUT_LENGTH];
...
void 
do_copyship(CHAR_DATA * ch, char *argument)
{
	SHIP_DATA *ship;
	SHIP_DATA *old;
	char 	arg[MAX_INPUT_LENGTH];
...
/* ARGSUSED */
void 
do_ships(CHAR_DATA * ch, char *argument)
{
	SHIP_DATA *ship;
	int 	count = 0;
...
/* ARGSUSED */
void 
do_speeders(CHAR_DATA * ch, char *argument)
{
	SHIP_DATA *ship;
	int 	count = 0;
...
/* ARGSUSED */
void 
do_allspeeders(CHAR_DATA * ch, char *argument)
{
	SHIP_DATA *ship;
	int 	count = 0;
...
/* ARGSUSED */
void 
do_allships(CHAR_DATA * ch, char *argument)
{
	SHIP_DATA *ship;
	int 	count = 0;
...
void 
do_leaveship(CHAR_DATA * ch, char *argument)
{
	ROOM_INDEX_DATA *fromroom;
	ROOM_INDEX_DATA *toroom;
	SHIP_DATA *ship;
...
void 
do_land(CHAR_DATA * ch, char *argument)
{
	char 	arg[MAX_INPUT_LENGTH];
	SHIP_DATA *ship;
	SHIP_DATA *target;
...
void 
do_accelerate(CHAR_DATA * ch, char *argument)
{
	int 	change;
	SHIP_DATA *ship;
	char 	buf[MAX_STRING_LENGTH];
...
void 
do_trajectory(CHAR_DATA * ch, char *argument)
{
	char 	buf[MAX_STRING_LENGTH];
	char 	arg2[MAX_INPUT_LENGTH];
...
void 
do_buyship(CHAR_DATA * ch, char *argument)
{
	long 	price;
	SHIP_DATA *ship;
	SHIP_DATA *ship_list;
...
void 
do_sellship(CHAR_DATA * ch, char *argument)
{
	long 	price;
	SHIP_DATA *ship;
...
void 
do_shipinfo(CHAR_DATA * ch, char *argument)
{
	SHIP_DATA *ship;
	SHIP_DATA *target;
	...
	if (fabsf(target->vx - ship->vx) > 500 + ship->sensor * 2 ||
	    fabsf(target->vy - ship->vy) > 500 + ship->sensor * 2 ||
	    fabsf(target->vz - ship->vz) > 500 + ship->sensor * 2) {
...
void 
do_autorecharge(CHAR_DATA * ch, char *argument)
{
	SHIP_DATA *ship;
	int 	recharge;
...
void 
do_autopilot(CHAR_DATA * ch, char *argument)
{
	SHIP_DATA *ship;
...
void 
do_openhatch(CHAR_DATA * ch, char *argument)
{
	SHIP_DATA *ship;
	char 	buf[MAX_STRING_LENGTH];
...
void 
do_closehatch(CHAR_DATA * ch, char *argument)
{
	SHIP_DATA *ship;
	char 	buf[MAX_STRING_LENGTH];
...
void 
do_hyperspace(CHAR_DATA * ch, char *argument)
{
	SHIP_DATA *ship;
	SHIP_DATA *eShip;
	char 	buf[MAX_STRING_LENGTH];
	...
	for (eShip = ship->starsystem->first_ship; eShip; eShip = eShip->next_in_starsystem) {
		...
		if (fabsf(eShip->vx - ship->vx) < 500
		    && fabsf(eShip->vy - ship->vy) < 500
		    && fabsf(eShip->vz - ship->vz) < 500) {
...
do_target(CHAR_DATA * ch, char *argument)
{
	...
	default:
		...
		if (fabsf(ship->vx - target->vx) > 5000 ||
		    fabsf(ship->vy - target->vy) > 5000 ||
		    fabsf(ship->vz - target->vz) > 5000) {
...
do_fire(CHAR_DATA * ch, char *argument)
{
	...
	if (ch->in_room->vnum == ship->gunseat && !str_prefix(argument, "lasers")) {
		...
		if (fabsf(target->vx - ship->vx) > 1000 ||
		    fabsf(target->vy - ship->vy) > 1000 ||
		    fabsf(target->vz - ship->vz) > 1000) {
	...
	if (ch->in_room->vnum == ship->gunseat && !str_prefix(argument, "rocket")) {
		...
		if (fabsf(target->vx - ship->vx) > 800 ||
		    fabsf(target->vy - ship->vy) > 800 ||
		    fabsf(target->vz - ship->vz) > 800) {
...
void 
do_calculate(CHAR_DATA * ch, char *argument)
{
	char 	arg1[MAX_INPUT_LENGTH];
	char 	arg2[MAX_INPUT_LENGTH];
	char 	arg3[MAX_INPUT_LENGTH];
	...
	if (ship->currjump == NULL) {
		...
		if (starsystem->star1 && strcmp(starsystem->star1, "") &&
		    fabsf(ship->jx - starsystem->s1x) < 300 &&
		    fabsf(ship->jy - starsystem->s1y) < 300 &&
		    fabsf(ship->jz - starsystem->s1z) < 300) {
			...
			return;
			    ...
			    fabsf(ship->jx - starsystem->s2x) < 300 &&
			    fabsf(ship->jy - starsystem->s2y) < 300 &&
		    fabsf(ship->jz - starsystem->s2z) < 300) {
			echo_to_cockpit(AT_RED, ship, "WARNING.. Jump coordinates too close to stellar object.");
			echo_to_cockpit(AT_RED, ship, "WARNING.. Hyperjump NOT set.");
			ship->currjump = NULL;
			return;
		} else if (starsystem->planet1 && strcmp(starsystem->planet1, "") &&
			    fabsf(ship->jx - starsystem->p1x) < 300 &&
			    fabsf(ship->jy - starsystem->p1y) < 300 &&
		    fabsf(ship->jz - starsystem->p1z) < 300) {
			echo_to_cockpit(AT_RED, ship, "WARNING.. Jump coordinates too close to stellar object.");
			echo_to_cockpit(AT_RED, ship, "WARNING.. Hyperjump NOT set.");
			ship->currjump = NULL;
			return;
		} else if (starsystem->planet2 && strcmp(starsystem->planet2, "") &&
			    fabsf(ship->jx - starsystem->p2x) < 300 &&
			    fabsf(ship->jy - starsystem->p2y) < 300 &&
		    fabsf(ship->jz - starsystem->p2z) < 300) {
			echo_to_cockpit(AT_RED, ship, "WARNING.. Jump coordinates too close to stellar object.");
			echo_to_cockpit(AT_RED, ship, "WARNING.. Hyperjump NOT set.");
			ship->currjump = NULL;
			return;
		} else if (starsystem->planet3 && strcmp(starsystem->planet3, "") &&
			    fabsf(ship->jx - starsystem->p3x) < 300 &&
			    fabsf(ship->jy - starsystem->p3y) < 300 &&
		    fabsf(ship->jz - starsystem->p3z) < 300) {
			echo_to_cockpit(AT_RED, ship, "WARNING.. Jump coordinates too close to stellar object.");
			echo_to_cockpit(AT_RED, ship, "WARNING.. Hyperjump NOT set.");
			ship->currjump = NULL;
			return;
...
void 
do_recharge(CHAR_DATA * ch, char *argument)
{
	int 	recharge;
	SHIP_DATA *ship;
...
void 
do_repairship(CHAR_DATA * ch, char *argument)
{
	char 	arg[MAX_INPUT_LENGTH];
	int 	change = 0;
	SHIP_DATA *ship;
...
void 
do_addpilot(CHAR_DATA * ch, char *argument)
{
	SHIP_DATA *ship;
...
void 
do_rempilot(CHAR_DATA * ch, char *argument)
{
	SHIP_DATA *ship;
...
void 
do_autotrack(CHAR_DATA * ch, char *argument)
{
	SHIP_DATA *ship;
...
/* ARGSUSED */
void 
do_pluogus(CHAR_DATA * ch, char *argument)
{
...
void 
do_drive(CHAR_DATA * ch, char *argument)
{
	int 	dir;
	SHIP_DATA *ship;
...
void 
do_chaff(CHAR_DATA * ch, char *argument)
{
	SHIP_DATA *ship;
...
ch_ret 
drive_ship(CHAR_DATA * ch, SHIP_DATA * ship, EXIT_DATA * pexit, int fall)
{
...
void 
do_callship(CHAR_DATA * ch, char *argument)
{
	SHIP_DATA *ship;
	int 	oldRoom;
	char 	arg[MAX_INPUT_LENGTH];
...
void 
miv(CHAR_DATA * ch, char *argument)
{
	char 	arg[MAX_INPUT_LENGTH];
...
void 
oiv(CHAR_DATA * ch, char *argument)
{
	char 	arg1[MAX_INPUT_LENGTH];
	char 	arg2[MAX_INPUT_LENGTH];
