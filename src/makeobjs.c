...
int calc_zeni(long double level, CHAR_DATA *killer) {
  ...
  zeni = (dice(get_rank_by_pl(level), zeni) + (dice(get_rank_by_pl(level), zeni) / 10 + dice(get_curr_lck(killer), zeni / 3)));
...
make_deathCertificate(CHAR_DATA *ch, CHAR_DATA *killer) {
  ...
  sprintf(buf,
          ...
          killer->name,
          get_rank(killer),
          killer->sex == SEX_MALE ? "Male" : killer->sex == SEX_FEMALE ? "Female"
                                                                       : "Neutral",
