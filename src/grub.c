...
struct { /* operand table - info about each
					 * operand   */
  int field;
  int op;
...
enum gr_field_type { /* enumerates the fields in the input
					 * record */
                     name,
                     sex,
                     class,
...
void rgrub_help(CHAR_DATA *ch) {
  send_to_char("Syntax:\n\r", ch);
  send_to_char(
      "rgrub st n lo hi - sector type search.\n\r"
      "   list room vnums between lo and hi that match n.\n\r",
      ch);
  send_to_char(
      "   e.g. rgrub st 6 901 969 - list all rooms in Olympus\n\r"
...
short go_wear_ext(long arg) { /* extract bit set in arg ignoring pos
					 * 1 */
  short cou;
...
bool go_parse_operator(CHAR_DATA *ch, char *pch, int *op_num) {
  ...
  if (go_fd[go_op[*op_num].field].num) {
    ...
    strcpy(go_op[*op_num].sval, pch); /* store str value in
							 * table */
  }
...
bool go_read(CHAR_DATA *ch, int dis_num, int op_num, int sor_ind,
  ...
  for (po = first_object; po; po = po->next) { /* Loop through all
							 * objects   */
...
void do_ogrub(CHAR_DATA *ch, char *argument) {
  ...
  if (isdigit(*arg1)) /* first arg is number of display
					 * lines   */
    dis_num = atoi(arg1);
  else {
  ...
  if (dis_num > MAX_DISPLAY_LINES) {
    pager_printf(ch,
                 "Sorry. You have requested more than %d display "
                 "lines.\n\r",
                 MAX_DISPLAY_LINES);
    return;
  }
...
bool gr_parse_operator(CHAR_DATA *ch, char *pch, int *op_num) {
  ...
  if (gr_fd[gr_op[*op_num].field].num) {
    ...
    gr_op[*op_num].nval = atol(pch); /* store num operand
							 * value in table */
  } else {
    if (strlen(pch) > MAX_FIELD_LENGTH) {
    ...
    strcpy(gr_op[*op_num].sval, pch); /* store str operand
							 * value in table */
  }
  (*op_num)++; /* operand now stored in table      */
  return true;
...
void gr_read(
  ...
  while (!feof(fp)) { /* read each input record    */
    ...
    if (res) { /* record is a match         */
      tot_match++;
      if (!title_sw && dis_num > 0) { /* print title if
							 * applicable */
        ch_printf(ch,
      ...
      if (tot_match <= dis_num) /* print record if
							 * applicable */
        ch_printf(ch,
...
void do_grub(CHAR_DATA *ch, char *argument) {
  ...
  if (isdigit(*arg1)) /* first argument is number of display
					 * lines */
    dis_num = atoi(arg1);
  else {
...
void do_showlayers(CHAR_DATA *ch, char *argument) {
  ...
  for (hash = 0; hash < MAX_KEY_HASH; hash++) /* loop thru
							 * obj_index_hash */
...
void diagnose_help(CHAR_DATA *ch) {
  ...
  send_to_char(
      "diagnose rf lo hi <flags> - room flag search.\n\r"
...
void diag_ins(OBJ_INDEX_DATA *p, int siz, OBJ_INDEX_DATA **f, CHAR_DATA *ch) {
  ...
  if (!f[siz - 1] || p->count > f[siz - 1]->count) /* don't bother looping
								 * thru f */
    while (cou < siz && ins < 0)                   /* should this vnum be
						 * insertted? */
      if (!f[cou++] || p->count > f[cou - 1]->count)
        ins = cou - 1; /* needs to go into pos "cou" */

  if (ins >= 0) {                         /* if vnum occurs more frequently */
    for (cou = siz - 1; cou > ins; cou--) /* open a slot in the
							 * table */
      f[cou] = f[cou - 1];
...
#define DIAG_RF_MAX_SIZE 5000
    ...
    if (!*arg2 || !*arg3 || !*argument) { /* empty arg gets help
							 * scrn */
      diagnose_help(ch);
      return;
    }
  ...
  if (!str_cmp(arg1, "of")) {
    ...
    if (*arg2) /* empty arg gets dft number */
      num = atoi(arg2);
    if (num > DIAG_MAX_SIZE || num < 1) { /* display num out of
							 * bounds */
      diagnose_help(ch);
      return;
    }
    ...
    for (cou = 0; cou < MAX_KEY_HASH; cou++) { /* loop thru
								 * obj_index_hash */
      if (obj_index_hash[cou])                 /* safety check */
        for (pObj = obj_index_hash[cou];       /* loop thru all pObjInd */
             pObj; pObj = pObj->next)
          diag_ins(pObj, num, freq, ch); /* insert pointer into
									 * list */
    }
...
*/
  ...
  if (!str_cmp(arg1, "mrc")) {
    ...
    if (!*arg2 || !*arg3 || !*arg4 || !*arg5 || !*arg6 || !isdigit(*arg2) || !isdigit(*arg3) || !isdigit(*arg4) || !isdigit(*arg5) || !isdigit(*arg6)) {
