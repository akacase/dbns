...
void decorate_room(ROOM_INDEX_DATA *room) {
  ...
  return; /* Return (with above till I can figure)
  ...
  /*
   * Below is the line that causes the uninitialized memory read
   * --Shaddai
   */
  sprintf(buf2, "%s\n\r", wordwrap(buf, 78));
...
move_char(CHAR_DATA *ch, EXIT_DATA *pexit, int fall) {
  ...
  if (!IS_NPC(ch)) {
    ...
    /*
     * Nuisance flag, makes them walk in random directions 50%
     * of the time -Shaddai
     */
  ...
  if (!pexit || !to_room) {
    if (drunk && ch->position != POS_MOUNTED && ch->in_room->sector_type != SECT_WATER_SWIM && ch->in_room->sector_type != SECT_WATER_NOSWIM && ch->in_room->sector_type != SECT_UNDERWATER && ch->in_room->sector_type != SECT_OCEANFLOOR && ch->in_room->sector_type != SECT_SPACE) {
  ...
  if (room_HBTC_not_partner(ch, to_room)) {
    send_to_char(
        "The person in there is not designated as your partner "
  ...
  if (!fall && !IS_NPC(ch)) {
    /*
     * Prevent deadlies from entering a nopkill-flagged area
     * from a non-flagged area, but allow them to move around if
     * already inside a nopkill area. - Blodkai
     *
     *
     * if (  IS_SET( to_room->area->flags, AFLAG_NOPKILL ) &&
     * !IS_SET( ch->in_room->area->flags, AFLAG_NOPKILL ) && (
     * IS_PKILL (ch) && !IS_IMMORTAL (ch) ) ) { set_char_color(
     * AT_MAGIC, ch ); send_to_char( "\n\rA godly force forbids
     * deadly characters from entering that area...\n\r", ch);
     * return (rNONE; })
     */
    if (in_room->sector_type == SECT_AIR || in_room->sector_type == SECT_SPACE || to_room->sector_type == SECT_AIR || to_room->sector_type == SECT_SPACE || IS_SET(pexit->exit_info, EX_FLY)) {
  ...
  /*
   * BIG ugly looping problem here when the character is mptransed
   * back to the starting room.  To avoid this, check how many chars
   * are in the room at the start and stop processing followers after
   * doing the right number of them.  -- Narn
   */
  if (!fall) {
    CHAR_DATA *fch;
...
void do_rest(CHAR_DATA *ch, char *argument) {
  if (xIS_SET((ch)->affected_by, AFF_PUSHUPS) || xIS_SET((ch)->affected_by, AFF_SHADOWBOXING) || xIS_SET((ch)->affected_by, AFF_ENDURING) || xIS_SET((ch)->affected_by, AFF_MEDITATION) || xIS_SET((ch)->affected_by, AFF_POWERCHANNEL) || xIS_SET((ch)->affected_by, AFF_POWERUPTRAIN) || xIS_SET((ch)->affected_by, AFF_OVERCHANNEL)) {
...
pullcheck(CHAR_DATA *ch, int pulse) {
  ...
  if (moveobj) {
    for (obj = room->first_content; obj; obj = obj_next) {
      ...
      /*
       * is the pull greater than the resistance of the
       * object?
       */
