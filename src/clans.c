...
bool is_leader(CHAR_DATA *ch) {
  ...
  if (!str_cmp(ch->name, clan->leader1) || !str_cmp(ch->name, clan->leader2) || !str_cmp(ch->name, clan->leader3) || !str_cmp(ch->name, clan->leader4) || !str_cmp(ch->name, clan->leader5) || !str_cmp(ch->name, clan->leader6))
    return true;
...
void write_council_list() {
  ...
  sprintf(filename, "%s", COUNCIL_LIST);
  fpout = fopen(filename, "w");
  if (!fpout) {
...
void load_councils() {
  ...
  sprintf(councillist, "%s", COUNCIL_LIST);
...
void do_induct(CHAR_DATA *ch, char *argument) {
  ...
  if ((ch->pcdata && ch->pcdata->bestowments && is_name("induct", ch->pcdata->bestowments)) || is_deity(ch) || is_leader(ch))
    ;
  else {
    send_to_char("Huh?\n\r", ch);
    return;
...
void do_outcast(CHAR_DATA *ch, char *argument) {
  ...
  if ((ch->pcdata && ch->pcdata->bestowments && is_name("outcast", ch->pcdata->bestowments)) || is_deity(ch) || is_leader(ch))
    ;
  else {
    send_to_char("Huh?\n\r", ch);
    return;
...
void do_showclan(CHAR_DATA *ch, char *argument) {
  ...
  if (clan->clan_type == CLAN_GUILD)
    ch_printf(ch, "   &wClass  : &W%d &w(&W%s&w)",
              clan->class,
              clan->class < MAX_PC_CLASS ? class_table[clan->class]->who_name : "unknown");
  send_to_char("\n\r", ch);
...
void do_orders(CHAR_DATA *ch, char *argument) {
  ...
  ch_printf(ch, "Deity      :  %s\n\rLeader     :  %s\n\rNumber One :  %s\n\rNumber Two :  %s\n\r",
            ...
            order->leader2,
            order->leader3);
  if (!str_cmp(ch->name, order->deity) || !str_cmp(ch->name, order->leader1) || !str_cmp(ch->name, order->leader2) || !str_cmp(ch->name, order->leader3))
...
void do_guilds(CHAR_DATA *ch, char *argument) {
  ...
  ch_printf(ch, "Leader:    %s\n\rNumber 1:  %s\n\rNumber 2:  %s\n\rMotto:     %s\n\r",
            ...
            guild->leader3,
            guild->motto);
  if (!str_cmp(ch->name, guild->deity) || !str_cmp(ch->name, guild->leader1) || !str_cmp(ch->name, guild->leader2) || !str_cmp(ch->name, guild->leader3))
    ch_printf(ch, "Members:   %d\n\r",
              guild->members);
  set_char_color(AT_HUNGRY, ch);
