...
char *const a_flags[] =
    {
        ...
        "sgod", "snamek", "icer2", "icer3", "icer4", "icer5", "pl_switch", "oozaru",
...
flag_string(int bitvector, char *const flagarray[]) {
  ...
  for (x = 0; x < 32; x++)
    if (IS_SET(bitvector, 1 << x)) {
      ...
      /*
       * don't catenate a blank if the last char is blank
       * --Gorog
       */
...
void do_goto(CHAR_DATA *ch, char *argument) {
  ...
  for (fch = in_room->first_person; fch; fch = fch_next) {
    ...
    /*
     * Experimental change by Gorog so imm's personal mobs
     * follow them
     */
...
void do_mset(CHAR_DATA *ch, char *argument) {
  ...
  /*
   * Disabled totaly because the rtg takes care of player zeni and
   * builder have *absoutly* no need for zeni. -Goku 09.28.04
   */
  ...
  if (!str_cmp(arg2, "clan")) {
    ...
    if (arg3[0] == '\0') {
      /*
       * Crash bug fix, oops guess I should have caught
       * this one :) But it was early in the morning :P
       * --Shaddai
       */
      if (victim->pcdata->clan == NULL)
        return;
      /*
       * Added a check on immortals so immortals don't
  ...
  if (!str_cmp(arg2, "flags")) {
    ...
    while (argument[0] != '\0') {
      ...
      if (value < 0 || value > MAX_BITS)
        ...
        if (IS_NPC(victim) && value == ACT_PROTOTYPE && get_trust(ch) < LEVEL_GREATER && !is_name("protoflag", ch->pcdata->bestowments))
          send_to_char("You cannot change the prototype flag.\n\r", ch);
        else if (IS_NPC(victim) && value == ACT_PROTOTYPE && xIS_SET(ch->act, PLR_QUEST_BUILDER) && victim->pIndexData->vnum > IMS_START_VNUM && victim->pIndexData->vnum < IMS_END_VNUM)
          send_to_char("You cannot change the prototype flag.\n\r", ch);
  ...
  if (!str_cmp(arg2, "speaks")) {
    ...
    while (argument[0] != '\0') {
      ...
      if (value == LANG_UNKNOWN)
        ...
        int valid_langs = LANG_COMMON | LANG_SAIYAN | LANG_HALFBREED | LANG_NAMEK | LANG_ANDROID | LANG_ICER | LANG_BIO_ANDROID | LANG_KAIO | LANG_DEMON | LANG_WIZARD;
...
void do_oset(CHAR_DATA *ch, char *argument) {
  ...
  if (get_trust(ch) < sysdata.level_modify_proto && !IS_OBJ_STAT(obj, ITEM_PROTOTYPE) && !xIS_SET(ch->act, PLR_QUEST_BUILDER)) {
  ...
  if (!str_cmp(arg2, "short")) {
    ...
    /*
     * Feature added by Narn, Apr/96 If the item is not
     * proto, add the word 'rename' to the keywords if
    ...
    while (argument[0] != '\0') {
      ...
      if (value < 0 || value > MAX_BITS)
        ...
        if (value == ITEM_PROTOTYPE && get_trust(ch) < LEVEL_GREATER && !is_name("protoflag", ch->pcdata->bestowments))
          send_to_char("You cannot change the prototype flag.\n\r", ch);
        else if (value == ITEM_PROTOTYPE && xIS_SET(ch->act, PLR_QUEST_BUILDER) && obj->pIndexData->vnum > IMS_START_VNUM && obj->pIndexData->vnum < IMS_END_VNUM)
          send_to_char("You cannot change the prototype flag.\n\r", ch);
        else {
...
void fold_area(AREA_DATA *tarea, char *filename, bool install) {
  ...
  for (vnum = tarea->low_m_vnum; vnum <= tarea->hi_m_vnum; vnum++) {
    ...
    if (pMobIndex->perm_str != 10 || pMobIndex->perm_int != 10 || pMobIndex->perm_dex != 10 || pMobIndex->perm_con != 10 || pMobIndex->perm_lck != 0 || pMobIndex->hitroll != 0 || pMobIndex->damroll != 0 || pMobIndex->race != 0 || pMobIndex->class != 0 || !xIS_EMPTY(pMobIndex->attacks) || !xIS_EMPTY(pMobIndex->defenses) || pMobIndex->height != 0 || pMobIndex->weight != 0 || pMobIndex->speaks != 0 || pMobIndex->speaking != 0 || pMobIndex->xflags != 0 || pMobIndex->numattacks != 0)
      complexmob = true;
    else
      complexmob = false;
    fprintf(fpout, "#%d\n", vnum);
...
void do_savearea(CHAR_DATA *ch, char *argument) {
  ...
  /*
   * Ensure not wiping out their area with save before load - Scryn
   * 8/11
   */
...
void do_installarea(CHAR_DATA *ch, char *argument) {
  ...
  for (tarea = first_build; tarea; tarea = tarea->next) {
    if (!str_cmp(tarea->filename, arg)) {
      ...
      /*
       * Fold area with install flag -- auto-removes
       * prototype flags
       */
...
void do_aset(CHAR_DATA *ch, char *argument) {
  ...
  if ((!proto && get_trust(ch) < sysdata.level_modify_proto) || (ch->pcdata->area != tarea && get_trust(ch) < LEVEL_GREATER)) {
...
void do_rlist(CHAR_DATA *ch, char *argument) {
  ...
  /*
   * I know the above code could be combined into 3 lines, but we
   * write code once and then read it many times. It is so much easier
   * to read without trying to figure out what opening parenthesis
   * belongs with what closing one.  -- Gorog
   */

  /*
   * Bah! The following code uses atoi(arg1) and atoi(arg2) without
   * even checking they are numeric or even exist. I put a fix for
   * this above. It also uses is_number(arg1) without checking if arg1
   * may be null. Caused a crash when the command was uses with a
   * single alpha arg. -- Gorog
   */

  if (tarea) {
    if (arg1[0] == '\0')          /* cleaned a big scary mess */
      lrange = tarea->low_r_vnum; /* here.
                                   * -Thoric */
    else
      lrange = atoi(arg1);
    if (arg2[0] == '\0')
...
void do_rdelete(CHAR_DATA *ch, char *argument) {
  ...
  if (get_trust(ch) < sysdata.level_modify_proto && (location->vnum < ch->pcdata->r_range_lo ||
...
void do_odelete(CHAR_DATA *ch, char *argument) {
  ...
  /* Does the player have the right to delete this room? */
  if (get_trust(ch) < sysdata.level_modify_proto && (obj->vnum < ch->pcdata->o_range_lo ||
...
void do_mdelete(CHAR_DATA *ch, char *argument) {
  ...
  /* Does the player have the right to delete this room? */
  if (get_trust(ch) < sysdata.level_modify_proto && (mob->vnum < ch->pcdata->m_range_lo ||
   ...
   * authority to delete it, so let's dump the thing. The function to
   * do it is in db.c so it can access the top_mob_index variable.
   */
  delete_mob(mob);
...
void do_opcopy(CHAR_DATA *ch, char *argument) {
  ...
  MPROG_DATA *source_oprog = NULL, *dest_oprog = NULL, *source_oprg = NULL,
             *dest_oprg = NULL;
...
void do_mpcopy(CHAR_DATA *ch, char *argument) {
  ...
  MPROG_DATA *source_mprog = NULL, *dest_mprog = NULL, *source_mprg = NULL,
             *dest_mprg = NULL;
...
void do_climate(CHAR_DATA *ch, char *argument) {
  ...
  /* add or remove neighboring weather systems */
    ...
    ch_printf(ch,
              "Climate may only be followed by one "
