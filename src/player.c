...
void 
do_gained(CHAR_DATA * ch, char *argument)
{
	if (IS_NPC(ch))
		return;
...
void 
do_pstatus(CHAR_DATA * ch, char *argument)
{
...
do_score(CHAR_DATA * ch, char *argument)
{
	...
	/*
	 * if (ch->level < 50) pager_printf(ch,"                   Align:
...
void 
do_description1(CHAR_DATA * ch, char *argument)
{
	if (IS_NPC(ch)) {
		send_to_char("Monsters are too dumb to do that!\n\r", ch);
		return;
...
void 
do_description2(CHAR_DATA * ch, char *argument)
{
	if (IS_NPC(ch)) {
		send_to_char("Monsters are too dumb to do that!\n\r", ch);
		return;
...
void 
do_description3(CHAR_DATA * ch, char *argument)
{
	if (IS_NPC(ch)) {
		send_to_char("Monsters are too dumb to do that!\n\r", ch);
		return;
...
void 
do_description4(CHAR_DATA * ch, char *argument)
{
	if (IS_NPC(ch)) {
		send_to_char("Monsters are too dumb to do that!\n\r", ch);
		return;
...
void 
do_description5(CHAR_DATA * ch, char *argument)
{
	if (IS_NPC(ch)) {
		send_to_char("Monsters are too dumb to do that!\n\r", ch);
		return;
...
void 
do_tag(CHAR_DATA * ch, char *argument)
{
	DESCRIPTOR_DATA *d;
...
void 
do_pk(CHAR_DATA * ch, char *argument)
{
	char arg[MAX_INPUT_LENGTH];
...
void 
do_setage(CHAR_DATA * ch, char *argument)
{
...
void 
do_setheight(CHAR_DATA * ch, char *argument)
{
	char arg[MAX_STRING_LENGTH];
	int value;
...
void 
do_setweight(CHAR_DATA * ch, char *argument)
{
	char arg[MAX_STRING_LENGTH];
	int value;
...
void 
do_reqbio(CHAR_DATA * ch, char *argument)
{
...
void 
do_aura_set(CHAR_DATA * ch, char *argument)
{
	sh_int auraColor;
	char colorWord[20];
	...
	if (!str_prefix(arg, "set")) {
		...
		if (!str_prefix(argument, "black")) {
			/*
			 * ch->pcdata->auraColorPowerUp = AT_BLACK; sprintf(
			 * colorWord, "%s", "Black" );
			 */
...
void 
do_corpse(CHAR_DATA * ch, char *argument)
{
	char buf[MAX_STRING_LENGTH];
	char arg[MAX_INPUT_LENGTH];
	...
	/*
	 * Avoids the potential for filling the room with hundreds of mob
	 * corpses
	 */
	if (IS_NPC(ch)) {
	...
	strcat(buf, ch->name);		/* Bug fix here by Samson 12-21-00 See
					 * below */
	for (obj = first_object; obj; obj = obj->next) {
		if (!nifty_is_name(buf, obj->short_descr))	/* Fix here - Samson
								 * 1-26-01 */
			continue;

		/*
		 * This will prevent NPC corpses from being retreived if the
		 * person has a mob's name
		 */
...
void 
do_sparcheck(CHAR_DATA * ch, char *argument)
{
...
void 
wss_scimitar(CHAR_DATA * ch, CHAR_DATA * victim, char *msg, int dam)
{
...
void 
wss_sword(CHAR_DATA * ch, CHAR_DATA * victim, char *msg, int dam)
{
	if (!str_cmp(msg, "normal")) {
...
void 
wss_lance(CHAR_DATA * ch, CHAR_DATA * victim, char *msg, int dam)
{
	if (!str_cmp(msg, "normal")) {
...
void 
wss_maul(CHAR_DATA * ch, CHAR_DATA * victim, char *msg, int dam)
{
	if (!str_cmp(msg, "normal")) {
