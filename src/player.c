...
void do_level(CHAR_DATA *ch, char *argument) {
  ...
  for (x = lowlvl; x <= hilvl; x++)
    ch_printf(ch, " (%2d) %12s%s\n\r", x, num_punct(exp_level(ch, x)),
              (x == ch->level) ? buf : (x == ch->level + 1) ? buf2
                                                            : " exp");
...
void do_title(CHAR_DATA *ch, char *argument) {
  ...
  if (strstr(argument, "(") || strstr(argument, ")") || strstr(argument, "[") || strstr(argument, "]") || strstr(argument, "<") || strstr(argument, ">") || strstr(argument, "{")) {
...
void do_delete(CHAR_DATA *ch, char *argument) {
  ...
  for (dold = first_descriptor; dold; dold = dold->next) {
    if (dold != ch->desc && (dold->character || dold->original) && !str_cmp(ch->name, dold->original ? dold->original->pcdata->filename : dold->character->pcdata->filename)) {
...
void do_aura_set(CHAR_DATA *ch, char *argument) {
  ...
  if (!str_prefix(arg, "show")) {
    ...
    sprintf(showBuf,
            ...
            "&B%-10s   &P%-10s   &C%-10s&D\n\r",
            "white", "darkred", "darkgreen", "brown", "darkblue", "purple", "cyan", "grey", "darkgrey", "red", "green", "yellow", "blue", "pink", "lightblue");
