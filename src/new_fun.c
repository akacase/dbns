...
bool is_splitformed(CHAR_DATA *ch) {
  if (xIS_SET(ch->affected_by, AFF_SPLIT_FORM) || xIS_SET(ch->affected_by, AFF_TRI_FORM) || xIS_SET(ch->affected_by, AFF_MULTI_FORM) || xIS_SET(ch->affected_by, AFF_BIOJR))
    return true;
...
bool str_ssh(const char *astr, const char *bstr) {
  ...
  for (ichar = 0; ichar <= sstr1; ichar++) {
    if ((astr[ichar] == '&' || astr[ichar] == '}' || astr[ichar] == '^') && LOWER(astr[ichar + 1]) != LOWER(bstr[ibstr])) {
      ichar++;
      continue;
    }
    /*
  ...
  while (!stop) {
    switch (number_range(1, 2)) {
      case 1:
        ...
        if (victim->pcdata->learned[sn] > 0 && ch->pcdata->learned[sn] < skill_table[sn]->skill_adept[ch->class] && ch->pl >= skill_table[sn]->skill_level[ch->class] && skill_table[sn]->skill_adept[ch->class] > 0 && skill_table[sn]->skill_level[ch->class] > 0 && sn != gsn_semiperfect && sn != gsn_perfect && sn != gsn_ultraperfect && sn != gsn_rescue)
          stop = true;
        snFound = true;
        break;
      case 2:
...
bool upgrade_player(CHAR_DATA *ch) {
  ...
  if (ch->pcdata->upgradeL == 3) {
    pager_printf(ch,
                 "Unauthorizing Bio.  Please see 'help bio' for"
                 " new guidelines.\n\r");
...
void obj_cost_recalc(OBJ_INDEX_DATA *obj) {
  ...
  for (iaf = obj->first_affect; iaf; iaf = iaf->next) {
    if (!iaf)
      break;
    if (iaf->location == APPLY_STR || iaf->location == APPLY_DEX || iaf->location == APPLY_INT || iaf->location == APPLY_CON)
