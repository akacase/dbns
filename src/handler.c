...
void affect_modify(CHAR_DATA *ch, AFFECT_DATA *paf, bool fAdd) {
  ...
  /*
         ...
         */
  if (!IS_NPC(ch) && saving_char != ch && (wield = get_eq_char(ch, WEAR_WIELD)) != NULL && get_obj_weight(wield) > (get_curr_str(ch) / 2)) {
    static int depth;
...
find_obj(CHAR_DATA *ch, char *argument, bool carryonly) {
  ...
  if (arg2[0] == '\0') {
    ...
    if (!obj)
      act(AT_PLAIN, IS_OBJ_STAT(container, ITEM_COVERING) ? "I see nothing like that beneath $p." : "I see nothing like that in $p.",
...
room_is_dnd(CHAR_DATA *ch, ROOM_INDEX_DATA *pRoomIndex) {
  ...
  for (rch = pRoomIndex->first_person; rch; rch = rch->next_in_room) {
    if (!IS_NPC(rch) && rch->pcdata && IS_IMMORTAL(rch) && IS_SET(rch->pcdata->flags, PCFLAG_DND) && get_trust(ch) < get_trust(rch))
      return rch;
  }
  return NULL;
}
...
bool can_see(CHAR_DATA *ch, CHAR_DATA *victim) {
  ...
  if (!IS_NPC(victim) && xIS_SET(victim->act, PLR_GHOST) && get_trust(ch) < victim->pcdata->ghost_level && ch->in_room->vnum == victim->in_room->vnum)
    return false;

  if (!IS_NPC(victim) && xIS_SET(victim->act, PLR_INCOG) && get_trust(ch) < victim->pcdata->incog_level && ch->in_room->vnum != victim->in_room->vnum)
    return false;
  ...
  if (!IS_AFFECTED(ch, AFF_trueSIGHT)) {
    ...
    if (room_is_dark(ch->in_room) && (!IS_AFFECTED(ch, AFF_INFRARED) && !is_android(ch) && !is_superandroid(ch) && !is_demon(ch) && !IS_NPC(ch)))
      return false;
...
bool can_astral(CHAR_DATA *ch, CHAR_DATA *victim) {
  if (victim == ch || !victim->in_room || !ch->in_room || xIS_SET(victim->in_room->room_flags, ROOM_PRIVATE) || xIS_SET(victim->in_room->room_flags, ROOM_SOLITARY) || xIS_SET(victim->in_room->room_flags, ROOM_NO_ASTRAL) || xIS_SET(victim->in_room->room_flags, ROOM_DEATH) || xIS_SET(victim->in_room->room_flags, ROOM_PROTOTYPE) || xIS_SET(ch->in_room->room_flags, ROOM_NO_RECALL) || xIS_SET(victim->in_room->room_flags, ROOM_NO_TELEPORT) || xIS_SET(ch->in_room->room_flags, ROOM_NO_TELEPORT_OUT) || IS_SET(victim->in_room->area->flags, AFLAG_NOTELEPORT) || IS_SET(ch->in_room->area->flags, AFLAG_NOTELEPORTOUT) || victim->in_room->area->areaPlanet <= 1 || room_is_dnd(ch, victim->in_room) || (carrying_dball(victim)) || (get_attmod(ch, victim) > 10 && (ch->kairank < 1 && ch->demonrank < 1) && !wearing_sentient_chip(ch)) || (is_kaio(ch) && is_kaio(victim) && victim->kairank > 0) || (is_demon(ch) && is_demon(victim) && victim->demonrank > 0) || (!IS_NPC(ch) && can_pk(victim)) || (IS_NPC(victim) && xIS_SET(victim->act, ACT_PROTOTYPE)))
    return false;
  ...
  if (destobj || (!destroom && !ch && (destobj = obj->in_obj) != NULL)) {
    for (otmp = obj->first_content; otmp; otmp = otmp_next) {
      ...
      if (destobj->item_type == ITEM_QUIVER && otmp->item_type != ITEM_PROJECTILE)
        continue;
      if ((destobj->item_type == ITEM_CONTAINER || destobj->item_type == ITEM_KEYRING || destobj->item_type == ITEM_QUIVER) && get_real_obj_weight(otmp) + get_real_obj_weight(destobj) > destobj->value[0])
        continue;
      obj_from_obj(otmp);
