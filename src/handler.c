...
char *affect_loc_name( int location )
{
    ...
    case APPLY_DEX:		return "speed";
    case APPLY_INT:		return "intelligence";
    case APPLY_CON:		return "constitution";
    case APPLY_LCK:		return "luck";
    case APPLY_ALLSTATS:		return "allstats";
	...
	case APPLY_NATURALAC:	return "natural ac";
    };
...
char *affect_bit_name( EXT_BV *vector )
{
    ...
    if ( xIS_SET(*vector, AFF_PL           )) strcat( buf, " powerlevel"    );
    if ( xIS_SET(*vector, AFF_KAIOKEN      )) strcat( buf, " kaioken"    );
    if ( xIS_SET(*vector, AFF_SSJ          )) strcat( buf, " ssj1"    );
    if ( xIS_SET(*vector, AFF_SSJ2         )) strcat( buf, " ssj2"    );
    if ( xIS_SET(*vector, AFF_SSJ3         )) strcat( buf, " ssj3"    );
    if ( xIS_SET(*vector, AFF_SSJ4         )) strcat( buf, " ssj4"    );
    if ( xIS_SET(*vector, AFF_SGOD         )) strcat( buf, " sgod"    );
    if ( xIS_SET(*vector, AFF_SNAMEK       )) strcat( buf, " snamek"    );
    if ( xIS_SET(*vector, AFF_ICER2        )) strcat( buf, " icer2"    );
    if ( xIS_SET(*vector, AFF_ICER3        )) strcat( buf, " icer3"    );
    if ( xIS_SET(*vector, AFF_ICER4        )) strcat( buf, " icer4"    );
    if ( xIS_SET(*vector, AFF_ICER5        )) strcat( buf, " icer5"    );
    if ( xIS_SET(*vector, AFF_GOLDENFORM   )) strcat( buf, " goldenform"    );
    if ( xIS_SET(*vector, AFF_PL_SWITCH    )) strcat( buf, " pl_switch"    );
    if ( xIS_SET(*vector, AFF_OOZARU       )) strcat( buf, " oozaru"    );
    if ( xIS_SET(*vector, AFF_GOLDEN_OOZARU)) strcat( buf, " goldenoozaru" );
    if ( xIS_SET(*vector, AFF_DEAD         )) strcat( buf, " dead"    );
    if ( xIS_SET(*vector, AFF_HEART        )) strcat( buf, " heart"    );
    if ( xIS_SET(*vector, AFF_TAG          )) strcat( buf, " tag"    );
    if ( xIS_SET(*vector, AFF_HYPER        )) strcat( buf, " hyper"    );
    if ( xIS_SET(*vector, AFF_SEMIPERFECT  )) strcat( buf, " semiperfect"    );
    if ( xIS_SET(*vector, AFF_PERFECT      )) strcat( buf, " perfect"    );
    if ( xIS_SET(*vector, AFF_ULTRAPERFECT )) strcat( buf, " ultraperfect"    );
    if ( xIS_SET(*vector, AFF_GROWTH       )) strcat( buf, " growth"    );
    if ( xIS_SET(*vector, AFF_GIANT        )) strcat( buf, " giant"    );
    if ( xIS_SET(*vector, AFF_SPLIT_FORM   )) strcat( buf, " split_form"    );
    if ( xIS_SET(*vector, AFF_TRI_FORM     )) strcat( buf, " tri_form"    );
    if ( xIS_SET(*vector, AFF_MULTI_FORM   )) strcat( buf, " multi_form"    );
    if ( xIS_SET(*vector, AFF_USSJ   )) strcat( buf, " ussj"    );
    if ( xIS_SET(*vector, AFF_USSJ2   )) strcat( buf, " ussj2"    );
    if ( xIS_SET(*vector, AFF_EXTREME )) strcat( buf, " extreme"    );
    if ( xIS_SET(*vector, AFF_EVIL_TRANS 	   )) strcat( buf, " thin");
    if ( xIS_SET(*vector, AFF_SUPER_TRANS 	   )) strcat( buf, " super");
    if ( xIS_SET(*vector, AFF_KID_TRANS 	   )) strcat( buf, " kid");
    if ( xIS_SET(*vector, AFF_MYSTIC 	   )) strcat( buf, " mystic");
    if ( xIS_SET(*vector, AFF_SUPERANDROID 	   )) strcat( buf, " superandroid");
    if ( xIS_SET(*vector, AFF_NO_GLOBAL_DROP 	   )) strcat( buf, " noglobaldrop");
    if ( xIS_SET(*vector, AFF_EVILBOOST		   )) strcat( buf, " evilboost");
    if ( xIS_SET(*vector, AFF_EVILSURGE		   )) strcat( buf, " evilsurge");
    if ( xIS_SET(*vector, AFF_EVILOVERLOAD	   )) strcat( buf, " eviloverload");
    if ( xIS_SET(*vector, AFF_MAKEOSTAR		   )) strcat( buf, " makeostar");
    if ( xIS_SET(*vector, AFF_BIOJR		   )) strcat( buf, " biojr");
...
char *extra_bit_name( EXT_BV *extra_flags )
{
    ...
    if ( xIS_SET(*extra_flags, ITEM_ANTI_SAIYAN)	) strcat( buf, " anti-saiyan"    );
    if ( xIS_SET(*extra_flags, ITEM_ANTI_HUMAN)	) strcat( buf, " anti-human"   );
    if ( xIS_SET(*extra_flags, ITEM_ANTI_HALFBREED)) strcat( buf, " anti-halfbreed" );
    if ( xIS_SET(*extra_flags, ITEM_ANTI_NAMEK)) strcat( buf, " anti-namek"  );
    if ( xIS_SET(*extra_flags, ITEM_ANTI_ANDROID)	) strcat( buf, " anti-android"   );
    if ( xIS_SET(*extra_flags, ITEM_ANTI_ICER)) strcat( buf, " anti-icer" );
    if ( xIS_SET(*extra_flags, ITEM_ANTI_BIO_ANDROID)) strcat( buf, " anti-bio-android" );
    ...
    if ( xIS_SET(*extra_flags, ITEM_RARE)	) strcat( buf, " rare"    );
    if ( xIS_SET(*extra_flags, ITEM_UNIQUE)	) strcat( buf, " unique"    );
    if ( xIS_SET(*extra_flags, ITEM_NOAUCTION)	) strcat( buf, " noauction " );
    if ( xIS_SET(*extra_flags, ITEM_NOGIVE)	) strcat( buf, " nogive " );
    if ( xIS_SET(*extra_flags, ITEM_NOQUIT)	) strcat( buf, " noquit " );
    if ( xIS_SET(*extra_flags, ITEM_NOSAC)	) strcat( buf, " nosac " );
    return ( buf[0] != '\0' ) ? buf+1 : "none";
}
