...
do_house (CHAR_DATA * ch, char *argument)
{
  ...
  if (!argument || argument[0] == '\0')
    {
      ...
      send_to_char_color
	("&C&G name <Desired Title of Room>\n\r desc\n\r", ch);
      ...
      if (IS_IMMORTAL (ch))
	{
	  ...
	  send_to_char_color
	    ("&C&G set <character name> <vnum> (apartment)\n\r", ch);
	  send_to_char_color
  ...
  if (homedata)
	...
	{
	  send_to_char_color
	    ("&C&wNot in a house room. Editing your first house room.\n\r",
	     ch);
  ...
  if (!str_cmp (arg, "name"))
	...
	{
	  send_to_char_color
	    ("&C&wHouse name <Desired Title of Room>\n\r", ch);
	  return;
	}
  ...
  if (!str_cmp (arg, "addroom"))
	...
	{
	  send_to_char_color
	    ("&C&wYou don't have enough to buy an additional room.\n\r", ch);
	  return;
	}
	...
	{
	  send_to_char_color
	    ("&C&wYou have the max allowable rooms in your house.\n\r", ch);
	  return;
	}
	...
	{
	  send_to_char_color
	    ("&C&wThere's already a room in that direction.\n\r", ch);
	  return;
	}
	...
	{
	  send_to_char_color
	    ("&C&wRoom couldn't be created. Please report to immortal.\n\r",
	     ch);
	  return;
	}
#ifdef BANK_INSTALLED
  ...
  if (!str_cmp (arg, "set"))
	...
	{
	  send_to_char_color
	    ("&C&wHouse set <character name> <vnum> (apartment)\n\r", ch);
	  send_to_char_color
      ...
      if (!str_cmp (arg3, "addroom"))
	{
	  ...
	  if (homedata->apartment)
	    {
	      send_to_char_color
		("&C&wApartments may only have one room.\n\r", ch);
	      return;
	    }
	  ...
	  if (i == MAX_HOUSE_ROOMS)
	    {
	      send_to_char_color
		("&C&wThey have the max allowable rooms.\n\r", ch);
	      return;
	    }
	  ...
	  if (!found)
	    {
	      send_to_char_color
		("&C&wThat vnum is not one of their house vnums.\n\r", ch);
	      return;
	    }
	  ...
	  if (!add_room (homedata, location, argument))
	    {
	      send_to_char_color
		("&C&wError: Room couldn't be created.\n\r", ch);
	      send_to_char_color
	  ...
	  send_to_char_color
	    ("&C&wAdditional room added to their house.\n\r", ch);
	  return;
	}
	...
	{
	  send_to_char_color
	    ("&C&wThe residence couldn't be created.\n\r", ch);
	  return;
	}
  ...
  if (!str_cmp (arg, "remove"))
	...
	{
	  send_to_char_color
	    ("&C&wNPC's don't have residences to begin with.\n\r", ch);
	  return;
	}
	...
	{
	  send_to_char_color
	    ("&C&wThe residence couldn't be removed.\n\r", ch);
	  return;
	}
...
do_gohome (CHAR_DATA * ch, char *argument)
{
  ...
  if (ch->pcdata->gohometimer > 0)
    {
      ch_printf (ch, "You can't go home for another %d minutes",
  ...
  switch (ch->position)
    ...
    case POS_STUNNED:
      send_to_char_color
	("&C&wYou can\'t do that, you\'re hurt too badly.\n\r", ch);
      return;
    case POS_SITTING:
    case POS_RESTING:
    case POS_SLEEPING:
...
do_residence (CHAR_DATA * ch, char *argument)
{
  ...
  if (!IS_IMMORTAL (ch))
    {
      send_to_char_color
	("&C&R### &W| &RPlayer Name &W| &RHousing Area &W| &R# of Rooms &W|\n\r",
	 ch);
      send_to_char_color
	("&C&W-----------------------------------------------\n\r", ch);
    }
  ...
  for (tmphome = first_home; tmphome; tmphome = tmphome->next)
	...
	if (tmphome->vnum[i] > 0)
      ...
      if (!IS_IMMORTAL (ch))
	ch_printf_color (ch,
			 "&C&c%3d &z| &c%11s &z| &c%12s &z| &c%10d &z|\n\r",
			 y, tmphome->name, area, z);
      else
	ch_printf_color (ch,
			 "&C&c%3d &z| &c%11s &z| &c%12s &z| &c%10d &z| &c%10d &z|\n\r",
...
do_accessories (CHAR_DATA * ch, char *argument)
{
  ...
  if (!homedata && !IS_IMMORTAL (ch))
    {
      send_to_char_color
	("&C&wYou need a house to get accessories for it.\n\r", ch);
      return;
    }
  ...
  if (arg[0] == '\0' || (is_number (arg) && arg2[0] == '\0'))
    {
      ...
      if (IS_IMMORTAL (ch))
	{
	  ...
	  send_to_char_color
	    ("&C&w <# of accessory> setprice <amount>\n\r", ch);
	}
  ...
  if (!is_number (arg))
      ...
      if (!str_cmp (arg, "list"))
	{
	  ...
	  if (!first_accessory)
	    {
	      send_to_char_color
		("&C&wThere are no accessories currently.\n\r", ch);
	      return;
	    }
	  ...
	  send_to_char_color
	    ("&C&g## &w|       &gAccessory Name       &w|    &gPrice    &w|\n\r",
	     ch);
	  send_to_char_color
	    ("&C&w-----------------------------------------------\n\r", ch);
		  ...
		  if ((mob = get_mob_index (tmpacc->vnum)) == NULL)
		    {
		      ...
		      bug (tmparg, 0);
		      ch_printf_color (ch,
				       "&C&g%2d &W| &RAccessory mob could not be found.\n\r",
				       i);
		      i++;
		      continue;
		    }
...
#ifndef HOUSE_MOBS
	      send_to_char_color
		("&C&wAccessories add <accessory vnum>\n\r", ch);
#else
	      send_to_char_color
		("&C&wAccessories add <accessory vnum> (mob)\n\r", ch);
#endif
	      return;
	    }
#ifdef HOUSE_MOBS
	  ...
	  for (tmpacc = first_accessory; tmpacc; tmpacc = tmpacc->next)
	      ...
	      if (tmpacc->vnum == atoi (arg2))
		{
		  if (mob && tmpacc->mob)
		    {
		      send_to_char_color
			("&C&wThat mob is already on accessories.\n\r", ch);
		      return;
		    }
		  else if (!mob && !tmpacc->mob)
		    {
		      send_to_char_color
			("&C&wThat object is already on accessories.\n\r",
			 ch);
		      return;
		    }
  ...
  if (!str_cmp (arg2, "show"))
      ...
      if (!tmpacc->mob)
	{
	  if ((obj = get_obj_index (tmpacc->vnum)) == NULL)
	    {
	      send_to_char_color
		("&C&wError: Accessory doesn't exist.\n\r", ch);
	      return;
	    }
      ...
      else
	{
	  ...
	  ch_printf_color (ch, "&C&gRace: &w%s\n\r",
			   (mob->race < MAX_NPC_RACE
			    && mob->race >=
			    0 ? npc_race[mob->race] : "unknown"));
	  ch_printf_color (ch, "&C&gClass: &w%s\n\r",
			   (mob->class < MAX_NPC_CLASS
			    && mob->class >=
			    0 ? npc_class[mob->class] : "unknown"));
	  ch_printf_color (ch, "&C&gAverage Hp: &w%d\n\r",
			   (!mob->hitnodice ? (mob->level * 8 +
					       number_range (mob->level *
							     ...
							     mob->level))
			    : (mob->hitnodice *
			       number_range (1, mob->hitsizedice)
			       + mob->hitplus)));
	  ch_printf_color (ch, "&C&gAverage AC: &w%d\n\r",
			   (mob->ac ? mob->ac : interpolate (mob->level,
							     100, -100)));
	  ch_printf_color (ch,
			   "&C&gNumber of Attacks(Per Round): &w%d\n\r",
			   mob->numattacks);
  ...
  if (!str_cmp (arg2, "buy"))
	...
	{
	  send_to_char_color
	    ("&C&wYou don't have enough to buy that accessory.\n\r", ch);
	  return;
	}
	  ...
	  if (ch->in_room->vnum == homedata->vnum[i])
	    {
	      location = ch->in_room;
	      send_to_char_color
		("&C&wPlacing accessory in the room you are currently in.\n\r",
		 ch);
	      break;
	    }
	}
      ...
      if (!location)
	{
      ...
      if ((tmpacc->mob
	   && (mobindex = get_mob_index (tmpacc->vnum)) == NULL)
	  || (!tmpacc->mob
	      && (objindex = get_obj_index (tmpacc->vnum)) == NULL))
	{
	  send_to_char_color
	    ("&C&wError: Accessory doesn't exist. Aborting.\n\r", ch);
	  return;
	}
#ifdef HOUSE_MOBS
      if (mobindex)
	{
	  ...
	  add_reset (location->area, 'M', 1, mobindex->vnum,
		     mobindex->count, location->vnum);
  ...
  if (!str_cmp (arg2, "setprice"))
	...
	{
	  send_to_char_color
	    ("&C&wAccessories <# of accessory> setprice <amount>\n\r", ch);
	  return;
	}
...
do_homebuy (CHAR_DATA * ch, char *argument)
{
  ...
  if (arg[0] == '\0' || (is_number (arg) && arg2[0] == '\0'))
      ...
      if (IS_IMMORTAL (ch))
	{
	  ...
	  send_to_char_color
	    ("&C&c <# of auction item> setbid <amount>\n\r", ch);
	  send_to_char_color
	    ("&C&c <# of auction item> bidincrement <percent>\n\r", ch);
	  send_to_char_color
	    ("&C&c <# of auction item> timeremainder <days> (<hours>)\n\r",
	     ch);
  ...
  if (!is_number (arg))
      ...
      if (!str_cmp (arg, "list"))
	{
	  ...
	  if (!first_homebuy)
	    {
	      send_to_char_color
		("&C&wThere are no residences currently on auction.\n\r", ch);
	      return;
	    }

	  send_to_char_color
	    ("&C&W                 Residence Auction\n\r", ch);
	  send_to_char_color
	  ...
	  for (tmpres = first_homebuy; tmpres; tmpres = tmpres->next)
		...
		{
		  sprintf (buf,
			   "Homebuy List: Item #%d's location doesn't exist.",
			   i);
		  bug (buf, 0);
		  ch_printf_color (ch,
				   "&C&c%2d &C| &cResidence could not be found.\n\r",
				   i);
		  i++;
		  continue;
		}
	      ...
	      else
		{
	      ...
	      ch_printf_color (ch,
			       "&C&c%2d &C| &c%14s &C| &c%12s &C| &c%11s &C| &c%10s &C| &c%2d &CDays &c%2d &CHrs &C|\n\r",
			       i, name, area, buf2, buf3, days,
      ...
      if (!str_cmp (arg, "add"))
	{
	  ...
	  if (arg2[0] == '\0' || !is_number (arg2))
	    {
	      send_to_char_color
		("&C&wHomebuy add <residence vnum> (apartment)\n\r", ch);
	      return;
	    }
	  ...
	  for (tmpres = first_homebuy; tmpres; tmpres = tmpres->next)
		...
		{
		  send_to_char_color
		    ("&C&wThat vnum is already on auction.\n\r", ch);
		  return;
		}
	    }
	  ...
	  if (!add_homebuy (ch, atoi (arg2), apartment, 0))
	    {
	      send_to_char_color
		("&C&wResidence couldn't be added to the auction.\n\r", ch);
	      return;
	    }

	  send_to_char_color
	    ("&C&wResidence added to the auction successfully.\n\r", ch);
	  return;
	}
  ...
  if (!str_cmp (arg2, "show"))
      ...
      if ((location = get_room_index (tmpres->vnum)) == NULL)
	{
	  ...
	  sprintf (buf,
		   "Homebuy show: Residence %d has a nonexistant location",
		   i);
	  bug (buf, 0);
	  return;
	}
      ...
      ch_printf_color (ch,
		       "&C&CTime Left: &c%d &CDays &c%d &CHours\n\r",
  ...
  if (!str_cmp (arg2, "bid"))
	...
	{
	  send_to_char_color
	    ("&C&wHomebuy <# of auction item> bid <amount>\n\r", ch);
	  return;
	}
      ...
      for (checkhome = first_homebuy; checkhome; checkhome = checkhome->next)
	{
	  if (!str_cmp (checkhome->bidder, ch->name))
	    {
	      send_to_char_color
		("&C&wYou are already bidding on a residence.\n\r", ch);
	      return;
	    }
	}
	...
	{
	  send_to_char_color
	    ("&C&wThe seller can't bid on their own item.\n\r", ch);
	  return;
	}
...
#endif
	{
      ...
      send_to_char_color
	("&C&wYou have successfully bid on that residence.\n\r", ch);
  ...
  if (!str_cmp (arg2, "setbid"))
	...
	{
	  send_to_char_color
	    ("&C&wHomebuy <# of auction item> setprice <amount>\n\r", ch);
	  return;
	}
  ...
  if (!str_cmp (arg2, "bidincrement"))
	...
	{
	  send_to_char_color
	    ("&C&wHomebuy <# of auction item> bidincrement <percent>\n\r",
	     ch);
	  return;
	}
  ...
  if (!str_cmp (arg2, "remove"))
	...
	{
	  send_to_char_color
	    ("&C&wResidence could not be removed from auction successfully.\n\r",
	     ch);
	  return;
	}

      send_to_char_color
	("&C&wResidence removed from auction successfully.\n\r", ch);
      return;
    }
...
do_sellhouse (CHAR_DATA * ch, char *argument)
{
  ...
  if (IS_NPC (ch))
    {
      send_to_char_color
	("&C&wMobs don't have residences to begin with.\n\r", ch);
      return;
    }
  ...
  if (!tmphome)
    {
      send_to_char_color
	("&C&wYou need a residence if your going to sell it.\n\r", ch);
      return;
    }
  ...
  if (!argument || argument[0] == '\0')
    {
      ...
      send_to_char_color
	("&C&w Places your residence on auction with default starting bid.\n\r",
	 ch);
  ...
  if (is_number (argument))
	  ...
	  || (!tmphome->apartment && atoi (argument) < MIN_HOUSE_BID))
	{
      ...
      if (!add_homebuy
	  (ch, tmphome->vnum[0], tmphome->apartment, atoi (argument)))
	{
	  send_to_char_color
...
remove_house (CHAR_DATA * ch)
{
  ...
  if (homedata->vnum[1] > 0)
      ...
      for (i = 1; i < MAX_HOUSE_ROOMS; i++)
	{
	  ...
	  for (mreset = addloc->area->first_reset; mreset;
	       mreset = mreset_next)
	    {
  ...
  if (!homedata->apartment
      && (key = get_obj_index (homedata->vnum[0])) != NULL)
    {
      delete_obj (key);
    }
...
bool
add_room (HOME_DATA * homedata, ROOM_INDEX_DATA * location, char *argument)
{
  ...
  sprintf (buf, "%s\'s Additional House Room %d",
	   capitalize (homedata->name), i);
  STRFREE (addloc->name);
...
bool
give_key (CHAR_DATA * ch, int vnum)
{
...
load_house_file (char *name)
{
  ...
  for (;;)
	...
	{
	  sprintf (buf,
		   "Load_house_file: bad section: %s in file %s.",
		   word, filename);
	  bug (buf, 0);
	  break;
	}
...
save_accessories ()
{
  ...
  if ((fpout = fopen (ACCESSORIES_FILE, "w")) == NULL)
    {
      bug ("Cannot open omeaccessories.dat for writing", 0);
  ...
  for (tmphome = first_homebuy; tmphome; tmphome = tmphomenext)
	  ...
	  && (bidder = load_player (tmphome->bidder)) != NULL)
	{
	  ...
	  if (bidder->pcdata->balance + bidder->gold < tmphome->bid)
	    {
	      if (bidder->pcdata->balance + bidder->gold >=
		  ((tmphome->bid * PENALTY_PERCENTAGE) / 100))
		{

		  if (bidder->pcdata->balance <
		      ((tmphome->bid * PENALTY_PERCENTAGE) / 100))
		    {
      ...
      if (tmphome->seller[0] == '\0'
	  || (seller = load_player (tmphome->seller)) == NULL)
	{
	  sprintf (buf,
      ...
      if (!bidder)
	{
	  ...
	  if (seller->pcdata->balance + seller->gold >=
	      ((tmphome->bid * PENALTY_PERCENTAGE) / 100))
	    {

	      if (seller->pcdata->balance <
		  ((tmphome->bid * PENALTY_PERCENTAGE) / 100))
		{

		  seller->gold -=
		    (((tmphome->bid *
		       PENALTY_PERCENTAGE) / 100) - seller->pcdata->balance);
      ...
      if (!set_house (bidder, tmphome->vnum, tmphome->apartment))
	{
	  ...
	  sprintf (buf,
		   "Homebuy_update: Residence could not be setup for vnum %d",
		   tmphome->vnum);
	  bug (buf, 0);
	  logoff (bidder);
	  logoff (seller);
      ...
      if (!remove_homebuy (tmphome))
	{
      ...
      if (bidder->desc)
	send_to_char_color
	  ("&C&wYou have successfully received your new residence.\n\r",
	   bidder);
      else
	add_loginmsg (bidder->name, 3);
      ...
      if (seller->desc)
	send_to_char_color
	  ("&C&wYou have successfully sold your residence.\n\r", seller);
      else
...
void
add_loginmsg (char *name, sh_int type)
{
  LMSG_DATA *newmsg = NULL;
...
check_loginmsg (CHAR_DATA * ch)
{
  ...
  switch (tmpmsg->type)
    ...
    case 4:
      ...
      break;
    default:
      sprintf (buf, "Error: Unknown Homebuy login msg: %d for %s.",
	       tmpmsg->type, ch->name);
      log_string (buf);
