...
void do_house(CHAR_DATA *ch, char *argument) {
  ...
  if (!str_cmp(arg, "set")) {
    ...
    if (arg2[0] == '\0' || arg3[0] == '\0' || (!is_number(arg3) && str_cmp(arg3, "addroom"))) {
  ...
  if (!str_cmp(arg2, "show")) {
    ...
    if (!tmpacc->mob) {
      ...
      ch_printf_color(ch, "&C&gRace: &w%s\n\r",
                      (mob->race < MAX_NPC_RACE && mob->race >=
                                                       0
                           ? npc_race[mob->race]
                           : "unknown"));
      ch_printf_color(ch, "&C&gClass: &w%s\n\r",
                      (mob->class < MAX_NPC_CLASS && mob->class >=
                                                         0
                           ? npc_class[mob->class]
                           : "unknown"));
...
bool set_house(CHAR_DATA *ch, int vnum, bool apartment) {
  ...
  if (apartment)
    location->description =
        STRALLOC("A room surrounds you, filled with the dirt and filth of previous tenants.\n\rA spray painted frame of the last tenant can be seen on the floor in a \n\rfar corner.\n\r");
  else
    location->description =
...
bool add_room(HOME_DATA *homedata, ROOM_INDEX_DATA *location, char *argument) {
  ...
  if ((pexit = make_exit(location, addloc, get_dir(argument))) == NULL || (rexit =
...
void homebuy_update() {
  ...
  for (tmphome = first_homebuy; tmphome; tmphome = tmphomenext) {
    ...
    if (tmphome->bidder[0] != '\0' && str_cmp(tmphome->bidder, "None") && (bidder = load_player(tmphome->bidder)) != NULL) {
