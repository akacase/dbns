	...
	bool pkill_ok(CHAR_DATA * ch, CHAR_DATA * victim)
{
	...
	if (ch->pcdata->clan && victim->pcdata->clan
	    && !xIS_SET(victim->act, PLR_PK1) && !xIS_SET(victim->act, PLR_PK2)) {
		...
		if (!clanbypass) {
			if (ch->pcdata->clan == victim->pcdata->clan) {
				send_to_char
				    ("You can't kill another member of your clan!\n\r",
				    ch);
				return false;
			}
			if (ch->pcdata->clan != victim->pcdata->clan) {
				...
				case ALLIANCE_ALLIED:
					send_to_char
					    ("Your clan is at peace with theirs!.\n\r",
					    ch);
					return false;
					break;
				case ALLIANCE_NEUTRAL:
					send_to_char
					    ("Your clan is in a state of neutrality with theirs!\n\r",
					    ch);
					return false;
					break;
	...
	if (xIS_SET(victim->act, PLR_BOUNTY) && victim->pcdata->bounty > 0
		...
		if (xIS_SET(victim->act, PLR_PK1) && !IS_HC(ch)) {
			...
			xSET_BIT(ch->act, PLR_PK2);
		} else if (!xIS_SET(victim->act, PLR_PK1)
		    && !xIS_SET(victim->act, PLR_PK2) && !IS_HC(victim)) {
			return false;
		}
		return true;
	}
	if (xIS_SET(victim->act, PLR_PK1) || xIS_SET(victim->act, PLR_WAR1)) {
		if ((float)ch->exp / victim->exp > 5) {
			send_to_char
			    ("They have a yellow PK flag and you are more than 5 times stronger.\n\r",
			    ch);
	...
	if (!xIS_SET(victim->act, PLR_PK1) && !xIS_SET(victim->act, PLR_PK2)
	    && !xIS_SET(victim->act, PLR_WAR1)
	    && !xIS_SET(victim->act, PLR_WAR2) && !IS_HC(victim)) {
...
violence_update(void)
{
	...
	for (ch = last_char; ch; lst_ch = ch, ch = gch_prev) {
		...
		if (xIS_SET((ch)->affected_by, AFF_EXSHADOWBOXING) && ch->position != POS_RESTING
			&& ch->position != POS_DEAD) {
			
			xREMOVE_BIT((ch)->affected_by, AFF_EXSHADOWBOXING);
			send_to_char("Your workout is cut short.\n\r", ch);
		}
		if (xIS_SET((ch)->affected_by, AFF_EXENDURING) && ch->position != POS_RESTING
			&& ch->position != POS_DEAD) {
		...
		if (xIS_SET((ch)->affected_by, AFF_INTEXSHADOWBOXING) && ch->position != POS_RESTING
			&& ch->position != POS_DEAD) {
			
			xREMOVE_BIT((ch)->affected_by, AFF_INTEXSHADOWBOXING);
			send_to_char("Your workout is cut short.\n\r", ch);
		}
		if (xIS_SET((ch)->affected_by, AFF_INTEXENDURING) && ch->position != POS_RESTING
			&& ch->position != POS_DEAD) {
...
multi_hit(CHAR_DATA * ch, CHAR_DATA * victim, int dt)
{
	...
	if (get_eq_char(ch, WEAR_DUAL_WIELD)) {
		dual_bonus =
		    IS_NPC(ch) ? (ch->level /
		    10) : (LEARNED(ch, gsn_dual_wield) / 10);
	...
	if (wield && !IS_SET(victim->immune, RIS_MAGIC)
		...
		for (aff = wield->pIndexData->first_affect; aff;
			...
			if (aff->location == APPLY_WEAPONSPELL
				...
				retcode =
				    (*skill_table[aff->modifier]->spell_fun)
				    (aff->modifier, (wield->level + 3) / 3, ch,
				    victim);
 ...
 */
int
ris_damage(CHAR_DATA * ch, int dam, int ris)
{
	sh_int modifier;
...
damage(CHAR_DATA * ch, CHAR_DATA * victim, int dam, int dt)
{
	...
	if (dam && dt != TYPE_UNDEFINED) {
		...
		else if (dt == (TYPE_HIT + DAM_POUND)
			    || dt == (TYPE_HIT + DAM_CRUSH)
			    || dt == (TYPE_HIT + DAM_STONE)
		    || dt == (TYPE_HIT + DAM_PEA))
			dam = ris_damage(victim, dam, RIS_BLUNT);
		else if (dt == (TYPE_HIT + DAM_STAB)
			    || dt == (TYPE_HIT + DAM_PIERCE)
			    || dt == (TYPE_HIT + DAM_BITE)
			    || dt == (TYPE_HIT + DAM_BOLT)
			    || dt == (TYPE_HIT + DAM_DART)
		    || dt == (TYPE_HIT + DAM_ARROW))
			dam = ris_damage(victim, dam, RIS_PIERCE);
		else if (dt == (TYPE_HIT + DAM_SLICE)
			    || dt == (TYPE_HIT + DAM_SLASH)
			    || dt == (TYPE_HIT + DAM_WHIP)
		    || dt == (TYPE_HIT + DAM_CLAW))
		...
		if ((bnopc > 0 && anopc > 0)
			...
			if (is_same_group(ch, victim)) {
				...
				act(AT_ACTION, "$n disbands from $N's group.",
				    ...
				    (ch->leader ==
				    victim) ? victim->master : ch->master,
				    TO_NOTVICT);
				if (ch->leader == victim)
					stop_follower(victim);
				else
	...
	if (dam && ch != victim
		...
		if (ch->race == 6 && !xIS_SET(ch->act, PLR_SPAR)
		    && !xIS_SET(victim->act, PLR_SPAR)) {
	...
	if (victim->position == POS_DEAD) {
		...
		if (!IS_NPC(ch)
		    && (is_bio(ch) || is_android(ch) || is_superandroid(ch))
		...
		if (!npcvict) {
			...
			if (!IS_NPC(ch) && !IS_NPC(victim)) {
				...
				if (!str_cmp(victim->name, ch->pcdata->hunting)
					...
					send_to_char
					    ("You may collect your earnings at any bounty officer.&R\n\r",
					    ch);
			...
			if (!IS_IMMORTAL(ch) && !IS_IMMORTAL(victim)) {
				...
				if (can_use_skill
					...
					learn_from_failure(victim,
						...
						if (IS_SET
						    (victim->pcdata->flags, PCFLAG_IMMORTALITY))
							immortal = true;
		...
		if (!IS_NPC(victim) && (preservation || biopres || immortal)) {
			...
			if (preservation) {
				act(AT_RED,
				    ...
				    victim, NULL, NULL, TO_ROOM);
				send_to_char
				    ("&RWith your final ounce of energy, you cough up an egg and launch it in to the air!",
				    victim);
...
is_safe(CHAR_DATA * ch, CHAR_DATA * victim, bool show_messg)
{
	...
	if (xIS_SET(victim->in_room->room_flags, ROOM_SAFE)) {
		if (show_messg) {
			...
			send_to_char
			    ("A magical force prevents you from attacking.\n\r",
			    ch);
		}
		return true;
	}
	...
	if (!xIS_SET(ch->act, PLR_SPAR) && !xIS_SET(victim->act, PLR_SPAR)) {
		if (get_timer(victim, TIMER_PKILLED) > 0) {
			if (show_messg) {
				...
				send_to_char
				    ("That character has died within the last 30 minutes.\n\r",
				    ch);
			}
			return true;
		}
		if (get_timer(ch, TIMER_PKILLED) > 0) {
			if (show_messg) {
				set_char_color(AT_GREEN, ch);
				send_to_char
				    ("You have been killed within the last 30 minutes.\n\r",
				    ch);
			}
			return true;
		}
 ...
 */
	...
	if (IS_NPC(ch)
	    || ch == victim
	    || ch->level >= LEVEL_IMMORTAL || xIS_SET(ch->act, PLR_ATTACKER)
...
raw_kill(CHAR_DATA * ch, CHAR_DATA * victim)
{
	...
	if (xIS_SET(victim->act, PLR_THIEF)) {
		...
		send_to_char
		    ("The gods have pardoned you for your thievery.\n\r",
		    victim);
	}
...
void
do_stopspar(CHAR_DATA * ch, char *argument)
{
	CHAR_DATA *victim;
...
void
do_spar(CHAR_DATA * ch, char *argument)
{
	char buf[MAX_STRING_LENGTH];
	...
	if (!IS_IMMORTAL(ch) && !IS_NPC(ch) && !IS_NPC(victim)) {
		if (!xIS_SET(ch->act, PLR_QUESTING)
			...
			send_to_char
			    ("You can't spar a player involved in a role playing event.\n\r",
			    ch);
			return;
		}
		if (xIS_SET(ch->act, PLR_QUESTING)
		    && !xIS_SET(victim->act, PLR_QUESTING)) {
			send_to_char
			    ("You can't spar a player not involved in a role playing event.\n\r",
			    ch);
			return;
		}
	}
	if (!IS_NPC(ch) && !IS_NPC(victim) && victim->hit < 86) {
		send_to_char
		    ("They need to rest a bit before sparring some more.\n\r",
		    ch);
		return;
	}
	if (!IS_NPC(ch) && !IS_NPC(victim) && ch->hit < 86) {
		send_to_char
		    ("You need to rest a bit before sparring some more.\n\r",
		    ch);
		return;
	}
	...
	if (ch->position == POS_RESTING || ch->position == POS_SLEEPING) {
		send_to_char
		    ("How do you propose to do that in your current state?\n\r",
		    ch);
		return;
	}
	if (ch->position == POS_FIGHTING
	...
	if (who_fighting(victim) != NULL) {
		send_to_char
		    ("It would not be honorable to interfere with some one else's battle.\n\r",
		    ch);
		return;
	}
	...
	if (!IS_NPC(ch) && !IS_NPC(victim) && !IS_HC(ch) && !IS_HC(victim)
		...
		send_to_char
		    ("You would not spar someone who is aligned that way.\n\r",
		    ch);
		return;
	}
	if (!victim->pcdata->HBTCPartner) {
		send_to_char
		    ("They are not accepting sparring partners at this time.\n\r",
		    ch);
		return;
	}
...
do_kill(CHAR_DATA * ch, char *argument)
{
	...
	if (IS_NPC(victim) && victim->master != NULL
	    && IS_IMMORTAL(victim->master) && !IS_IMMORTAL(ch)) {
		send_to_char("Cheatz.\n\r", ch);
		return;
	}
	...
	if (!IS_IMMORTAL(ch) && !IS_NPC(ch) && !IS_NPC(victim)) {
		if (!xIS_SET(ch->act, PLR_QUESTING)
		    && xIS_SET(victim->act, PLR_QUESTING)) {
			send_to_char
			    ("You can't attack a player involved in a role playing event.\n\r",
			    ch);
			return;
		}
		if (xIS_SET(ch->act, PLR_QUESTING)
		    && !xIS_SET(victim->act, PLR_QUESTING)) {
			send_to_char
			    ("You can't attack a player not involved in a role playing event.\n\r",
			    ch);
			return;
		}
	}
	...
	if (!IS_NPC(ch) && !IS_NPC(victim) && strcmp(argument, "now")) {
		send_to_char
		    ("You must type 'kill <person> now' if you want to kill a player.\n\r",
		    ch);
		return;
	}
	if (!IS_NPC(ch) && !IS_NPC(victim))
	...
	if (!IS_NPC(victim)) {
		sprintf(log_buf, "PLAYER COMBAT: %s[%s] vs. %s[%s].",
		    ch->name, !xIS_SET(ch->act,
		    PLR_SPAR) ? "DEADLY" : "SPARING",
		    victim->name, !xIS_SET(victim->act,
		    PLR_SPAR) ? "DEADLY" :
		    "SPARING");
...
do_flee(CHAR_DATA * ch, char *argument)
{
	...
	if (!IS_NPC(ch) && !IS_NPC(who_fighting(ch))
	    && xIS_SET(ch->act, PLR_SPAR)
	    && xIS_SET(who_fighting(ch)->act, PLR_SPAR)) {
...
do_slay(CHAR_DATA * ch, char *argument)
{
	...
	if (arg[0] == '\0') {
		...
		send_to_char
		    ("Types: Skin, Slit, Immolate, Demon, Shatter, 9mm, Deheart, Pounce, Cookie, Fslay.\n\r",
		    ch);
		return;
	}
	if ((victim = get_char_room(ch, arg)) == NULL) {
...
void
do_fslay(CHAR_DATA * ch, char *argument)
{
	{
		CHAR_DATA *victim;
		char arg[MAX_INPUT_LENGTH];
		...
		if (arg[0] == '\0') {
			send_to_char("Syntax: [Char] [Type]\n\r", ch);
			send_to_char
			    ("Types: Skin, Slit, Immolate, Demon, Shatter, 9mm, Deheart, Pounce, Cookie, Fslay, Lightning.\n\r",
			    ch);
			return;
		}
		if ((victim = get_char_room(ch, arg)) == NULL) {
