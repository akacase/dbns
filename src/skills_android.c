...
void do_androidfuse(CHAR_DATA *ch, char *argument) {
  ...
  ch_printf(dch,
            "&G%s suddenly breaks apart. All of %'s body pieces drop the ground\n\r"
  ...
  ch_printf(sch,
            "&RYou suddenly lose control of your body. Your vision begins to twist\n\r"
...
void do_rocket_punch(CHAR_DATA *ch, char *argument) {
  ...
  if (can_use_skill(ch, number_percent(), gsn_rocket_punch)) {
    ...
    act(AT_DGREY,
        "$N is smashed by the flying fist, "
        "which quickly returns and reattaches to your arm. &W[$t]",
    ...
    act(AT_DGREY,
        "You are smashed by the flying fist, "
        "which quickly returns and reattaches to $n's arm. &W[$t]",
...
void do_drp(CHAR_DATA *ch, char *argument) {
  ...
  if (can_use_skill(ch, number_percent(), gsn_drp)) {
    ...
    act(AT_DGREY,
        "$N is smashed by the flying fists, "
        "which quickly return and reattach to your arms. &W[$t]",
        ch, num_punct(dam), victim, TO_CHAR);
    act(AT_DGREY, "Both of $n's fists detach, powerful rockets firing them toward you.", ch, NULL, victim, TO_VICT);
    act(AT_DGREY,
        "You are smashed by the flying fists, "
        "which quickly return and reattach to $n's arms. &W[$t]",
        ch, num_punct(dam), victim, TO_VICT);
...
void do_evolve(CHAR_DATA *ch, char *argument) {
  ...
  if (str_prefix(argument, "semi-perfect form") && str_prefix(argument, "perfect form") && str_prefix(argument, "ultra-perfect form")) {
    send_to_char("You can't evolve into that.\n\r", ch);
    return;
  }
  if ((!str_prefix(argument, "semi-perfect form") && ch->pcdata->learned[gsn_semiperfect] <= 0) || (!str_prefix(argument, "perfect form") && ch->pcdata->learned[gsn_perfect] <= 0) || (!str_prefix(argument, "ultra-perfect form") && ch->pcdata->learned[gsn_ultraperfect] <= 0)) {
  ...
  if (!str_prefix(argument, "semi-perfect form") && ch->pcdata->learned[gsn_semiperfect] > 0) {
    ...
    act(AT_PURPLE,
        "Your horns shift closer together, your features becoming more humanoid as "
        "you grow in size and complete your evolution to a Semi-Perfect form.",
        ch, NULL, NULL, TO_CHAR);
    act(AT_PURPLE,
        "$n's horns shift closer together, $s features becoming more humanoid as "
        "$e grows in size and completes $s evolution to a Semi-Perfect form.",
  ...
  if (!str_prefix(argument, "perfect form") && ch->pcdata->learned[gsn_perfect] > 0) {
    ...
    xREMOVE_BIT(ch->affected_by, AFF_ULTRAPERFECT);
    act(AT_PURPLE,
        "You surge with energy as your body becomes more compact, efficient, and almost "
        "human.  Your horns fuse into a kind of hat, your tail retracts up under your "
        "wings as you evolve to a Perfect form.",
        ch, NULL, NULL, TO_CHAR);
    act(AT_PURPLE,
        "$n surges with energy as $s body becomes more compact, efficient, and almost "
        "human.  $n's horns fuse into a kind of hat, $s tail retracts up under $s wings "
        "as $e evolves to a Perfect form.",
        ch, NULL, NULL, TO_NOTVICT);
    ch->pl = ch->exp * 18;
  ...
  if (!str_prefix(argument, "ultra-perfect form") && ch->pcdata->learned[gsn_ultraperfect] > 0) {
    ...
    act(AT_PURPLE,
        "You draw on newfound power, your body moving with perfect grace and "
        "efficiency, your entire form free from blemish or impurity.  You calmly stand "
        "in this most human of forms, having evolved to an Ultra-Perfect being.",
        ch, NULL, NULL, TO_CHAR);
    act(AT_PURPLE,
        "$n draws on newfound power, $s body moving with perfect grace and efficiency, "
        "$s entire form free from blemish or impurity.  $n calmly stands in this most "
        "human of forms, having evolved to an Ultra-Perfect being.",
        ch, NULL, NULL, TO_NOTVICT);
    ch->pl = ch->exp * 27;
...
void do_ki_burst(CHAR_DATA *ch, char *argument) {
  ...
  if (can_use_skill(ch, number_percent(), gsn_ki_burst)) {
    ...
    act(z, "The aura explodes outward, forming a deadly sphere that sends you reeling. &W[$t]", ch, num_punct(dam), victim, TO_VICT);
    act(z, "$n clenches $s fists as a black and purple aura flares up around $m.", ch, NULL, victim, TO_NOTVICT);
    act(z, "The aura explodes outward, forming a deadly sphere that sends $N reeling. &W[$t]", ch, num_punct(dam), victim, TO_NOTVICT);
...
void do_ssd_bomber(CHAR_DATA *ch, char *argument) {
  ...
  if (can_use_skill(ch, number_percent(), gsn_ssd_bomber)) {
    ...
    act(AT_RED,
        "You clasp your hands forward, as if charging an "
        ...
        ch, num_punct(dam), victim, TO_CHAR);
    act(AT_RED,
        "$n clasps $s hands forward, as if charging an "
        "enery ball.  Suddenly, it surges with incredible "
        "power, entangled in a spider-web-like sphere of "
