...
void do_nlist (CHAR_DATA *ch, char *argument)
{
	...
	if (is_number(argument))	 /* first, count the number of notes */
	{
        ...
        if( showFrom && argument[0] == '\0' )
        {
          send_to_char_color( "&WYou must provide a name for 'from'.\n\r", ch );
          return;
        }
        else
        if( (showTo || showFrom) && argument[0] != '\0' )
        {
...
void do_global_boards (CHAR_DATA *ch, char *argument)
{
	...
	if (!argument[0]) /* show boards */
	{
		...
		ch_printf(ch,"\n\r");
                if( IS_IMMORTAL( ch ) )
                {
		  send_to_char_color ("&RNum         Name Unread Personal Clan Immort Description&w\n\r"
		                      "&r=== ============ ====== ======== ==== ====== ===========&w\n\r", ch);
                }
                else
                {
		  send_to_char_color ("&RNum         Name Unread Personal Clan Description&w\n\r"
		                      "&r=== ============ ====== ======== ==== ===========&w\n\r", ch);
                }
		for (i = 0; i < MAX_BOARD; i++)
		{
	...
	if (is_number(argument))
	{
		...
		if (count == number) /* found the board.. change to it */
		{
			...
			if ( (get_trust(ch) < boards[i].write_level)
			     ...
			     || ( !xIS_SET(ch->act, PLR_CAN_CHAT) && !IS_IMMORTAL(ch)
			          && boards[i].access == BOARD_AUTHED_BIO ) )
