...
void interpret(CHAR_DATA *ch, char *argument) {
  ...
  if (!cmd) {
    ...
    for (cmd = command_hash[LOWER(command[0]) % 126]; cmd; cmd = cmd->next)
      if (!str_prefix(command, cmd->name) && (cmd->level <= trust || (!IS_NPC(ch) && ch->pcdata->council && is_name(cmd->name, ch->pcdata->council->powers) && cmd->level <= (trust + MAX_CPD)) || (!IS_NPC(ch) && ch->pcdata->bestowments && ch->pcdata->bestowments[0] != '\0' && is_name(cmd->name, ch->pcdata->bestowments) && cmd->level <= (trust + sysdata.bestow_dif)))) {
        found = true;
        break;
      }
        ...
        if (!strcmp(m_data[i].sectionNum, command)) {
          if (!strncmp(m_data[i].charChoice, argument, 1)) /* just check the 1st char */
          {
            ...
            switch (m_data[i].cmdArgs) {
              ...
              case 0:
              default:
                sprintf(newcommand, "%s", m_data[i].cmdString);
            }
  ...
  if (tmptime > 1500000) {
    sprintf(log_buf, "[*****] LAG: %s: %s %s (R:%d S:%lld.%06ld)", ch->name,
