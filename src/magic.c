...
int ch_slookup(CHAR_DATA *ch, const char *name) {
  ...
  for (sn = 0; sn < top_sn; sn++) {
    if (!skill_table[sn]->name)
      break;
    if (ch->pcdata->learned[sn] > 0 && ch->exp >= skill_table[sn]->skill_level[ch->class] && LOWER(name[0]) == LOWER(skill_table[sn]->name[0]) && !str_prefix(name, skill_table[sn]->name))
      return sn;
  }
...
int ch_bsearch_skill_prefix(CHAR_DATA *ch, const char *name, int first, int top) {
  ...
  for (;;) {
    ...
    if (LOWER(name[0]) == LOWER(skill_table[sn]->name[0]) && !str_prefix(name, skill_table[sn]->name) && ch->pcdata->learned[sn] > 0 && ch->exp >= skill_table[sn]->skill_level[ch->class])
      return sn;
    if (first >= top)
      return -1;
    if (strcmp(name, skill_table[sn]->name) < 1)
...
int ch_bsearch_skill_exact(CHAR_DATA *ch, const char *name, int first, int top) {
  ...
  for (;;) {
    ...
    if (!str_cmp(name, skill_table[sn]->name) && ch->pcdata->learned[sn] > 0 && ch->exp >= skill_table[sn]->skill_level[ch->class])
      return sn;
    if (first >= top)
      return -1;
    if (strcmp(name, skill_table[sn]->name) < 1)
...
spell_affect(int sn, int level, CHAR_DATA *ch, void *vo) {
  ...
  if (!groupsp && !areasp) {
    ...
    if ((saf = skill->affects) && !saf->next && saf->location == APPLY_STRIPSN && !is_affected(victim, dice_parse(ch, level, saf->modifier))) {
      failed_casting(skill, ch, victim, NULL);
      return rSPELL_FAILED;
    }
  ...
  switch (SPELL_ACTION(skill)) {
    ...
    case SA_CREATE:
      if (SPELL_FLAG(skill, SF_WATER)) { /* create water */
        ...
        water = UMIN((skill->dice ? dice_parse(ch, level, skill->dice) : level) * (weath->precip >= 0 ? 2 : 1),
