...
bool mprog_seval
    args((char *lhs, char *opr, char *rhs,
          CHAR_DATA *mob));
bool mprog_veval args((int lhs, char *opr, int rhs,
                       CHAR_DATA *mob));
bool mprog_vevall args((long double lhs, char *opr, long double rhs,
                        CHAR_DATA *mob));
 ...
 * If there are errors, then return true otherwise return boolean 1,0
...
doneargs:
  ...
  *p = '\0';
  ...
  if (!str_cmp(chck, "ovnumroom")) {
    ...
    rhsvl = is_number(rval) ? atoi(rval) : -1;
    /*
		 * Changed below from 1 to 0 so can check for == no items
		 * Shaddai
		 */
    if (rhsvl < 0)
      rhsvl = 0;
  ...
  if (chkchar) {
    ...
    if (!str_cmp(chck, "multi")) {
      ...
      for (ch = first_char; ch; ch = ch->next)
        if (!IS_NPC(chkchar) && !IS_NPC(ch) && ch->desc && chkchar->desc && QUICKMATCH(ch->desc->host, chkchar->desc->host))
          lhsvl++;
      rhsvl = atoi(rval);
      if (rhsvl < 0)
        rhsvl = 0;
    ...
    if (!str_cmp(chck, "race")) {
      ...
      if (is_android(chkchar) ||
          wearing_sentient_chip(chkchar))
    ...
    /*
		 * Check added to see if the person isleader of == clan
		 * Shaddai
		 */
...
void mprog_driver(char *com_list, CHAR_DATA *mob, CHAR_DATA *actor,
  ...
  while (true) {
    /*
		 * With these two lines, cmnd becomes the current line from
		 * the prog, and command_list becomes everything after that
		 * line.
		 */
    cmnd = command_list;
    ...
    /*
		 * This is the complicated part.  Act on the returned value
		 * from mprog_do_command according to the current logic
		 * state.
		 */
    switch (result) {
      case COMMANDOK:
      ...
      case IFtrue:
        /*
			 * An if was evaluated and found true.  Note that we
			 * are in an if section and that we want to execute
			 * it.
			 */
        iflevel++;
        if (iflevel == MAX_IFS) {
      ...
      case IFfalse:
        /*
			 * An if was evaluated and found false.  Note that
			 * we are in an if section and that we don't want to
			 * execute it unless we find an or that evaluates to
			 * true.
			 */
        iflevel++;
        if (iflevel == MAX_IFS) {
      ...
      case ORtrue:
        /*
			 * An or was evaluated and found true.  We should
			 * already be in an if section, so note that we want
			 * to execute it.
			 */
      ...
      case ORfalse:
        /*
			 * An or was evaluated and found false.  We should
			 * already be in an if section, and we don't need to
			 * do much.  If the if was true or there were/will
			 * be other ors that evaluate(d) to true, they'll
			 * set do_if to true.
			 */
        if (!ifstate[iflevel][IN_IF]) {
          progbug("Unmatched or", mob);
      ...
      case FOUNDELSE:
        /*
			 * Found an else.  Make sure we're in an if section,
			 * bug out if not. If this else is not one that we
			 * wish to ignore, note that we're now in an else
			 * section, and look at whether or not we executed
			 * the if section to decide whether to execute the
			 * else section.  Ca marche bien.
			 */
        if (ignorelevel > 0)
          continue;
      ...
      case FOUNDENDIF:
        /*
			 * Hmm, let's see... FOUNDENDIF must mean that we
			 * found an endif. So let's make sure we were
			 * expecting one, return if not.  If this endif
			 * matches the if or else that we're executing, note
			 * that we are now no longer executing an if.  If
			 * not, keep track of what we're ignoring.
			 */
        if (!(ifstate[iflevel][IN_IF] || ifstate[iflevel][IN_ELSE])) {
...
int mprog_do_command(char *cmnd, CHAR_DATA *mob, CHAR_DATA *actor,
  ...
  /*
	 * Isolate the first word of the line, it gives us a clue what we
	 * want to do.
	 */
  ...
  if (!str_cmp(firstword, "if")) {
    /*
		 * Ok, we found an if.  According to the boolean 'ignore',
		 * either ignore the ifcheck and report that back to
		 * mprog_driver or do the ifcheck and report whether it was
		 * successful.
		 */
    if (ignore)
      return IFIGNORED;
  ...
  if (!str_cmp(firstword, "or")) {
    /*
		 * Same behavior as with ifs, but use the boolean
		 * 'ignore_ors' to decide which way to go.
		 */
    if (ignore_ors)
      return ORIGNORED;
    else
  ...
  /*
	 * Ok, didn't find an if, an or, an else or an endif. If the command
	 * is in an if or else section that is not to be performed, the
	 * boolean 'ignore' is set to true and we just return.  If not, we
...
void set_supermob(OBJ_DATA *obj) {
  ...
  /*
	 ...
	 * object, and not just supermob's vnum
	 */
...
void oprog_greet_trigger(CHAR_DATA *ch) {
  ...
  for (vobj = ch->in_room->first_content; vobj; vobj = vobj->next_content)
    if (HAS_PROG(vobj->pIndexData, GREET_PROG)) {
      set_supermob(vobj); /* not very efficient to do
						 * here */
...
void rset_supermob(ROOM_INDEX_DATA *room) {
  ...
  if (room) {
    ...
    /*
		 * Added by Jenny to allow bug messages to show the vnum of
		 * the room, and not just supermob's vnum
		 */
